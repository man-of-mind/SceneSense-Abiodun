...
class _StaticTrafficLightAnchor:
    """Minimal actor-like anchor loaded from traffic_lights_data.json when
    the live CARLA world returns no traffic-light actors (e.g. sync-world race)."""
...
def _load_static_traffic_light_anchor(
    requested_id: str,
) -> "Optional[_StaticTrafficLightAnchor]":
    ...
    for entry in rows if isinstance(rows, list) else []:
        ...
        od_id = str(entry.get("opendrive_id", "")).strip()
        if od_id:
            candidates.add(od_id)
        ...
        if isinstance(rotation, dict) and "yaw" in rotation:
            yaw_deg = _safe_float(rotation.get("yaw"), 0.0)
        elif "yaw" in entry:
            yaw_deg = _safe_float(entry.get("yaw"), 0.0)
        else:
            yaw_deg = 0.0
        return _StaticTrafficLightAnchor(
            actor_id=int(entry.get("id", 0)),
            location=carla.Location(
                x=_safe_float(loc_data.get("x"), 0.0),
                y=_safe_float(loc_data.get("y"), 0.0),
                z=_safe_float(loc_data.get("z"), 0.0),
            ),
            yaw_deg=yaw_deg,
            opendrive_id=od_id,
        )
...
def _resolve_traffic_light_with_fallback(
    ...
    """Resolve a traffic-light actor with retry + static-JSON fallback.

    When the first client holds the sync-world tick, a second client's initial
    world.get_actors() call may return an empty list.  We retry up to `retries`
    times; if actors are still absent we fall back to traffic_lights_data.json.
    If actors are present but the requested id is not among them, we raise
    immediately (no point retrying a genuinely absent TL).
    """
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            try:
                ...
                live_count = 0
            if live_count > 0:
                raise  # TL not found but others exist → genuinely absent
            if attempt < retries:
                print(
                    f"[CARLA] No live traffic-light actors visible while resolving "
                    f"{requested_id!r}; retrying {attempt}/{retries}..."
                )
                time.sleep(retry_delay_s)

    fallback = _load_static_traffic_light_anchor(requested_id)
    if fallback is not None:
        print(
            f"[CARLA] Live actor discovery failed for {requested_id!r}; "
            "using static anchor from traffic_lights_data.json."
        )
...
def run_client(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        traffic_light = _resolve_traffic_light_with_fallback(world, str(args.traffic_light_id))
