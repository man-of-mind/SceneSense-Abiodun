#!/usr/bin/env python3
"""Two per-speed error(Y) plots — one for INTERSECTION observations, one for STRAIGHT-ROAD — to check the
speed/latency story holds within each road state. Same opportunity-window data as the main per-speed plot,
split by road state (CARLA Town10 map at each target's GT position). Requires CARLA up (Town10)."""
...
MIN_N = 10   # per (road-state x speed) cell — lower than the pooled plot since we're splitting the data
...
def road_state(x,y):
    ...
    return "intersection" if (wp and wp.is_junction) else "straight"
...
for rs,fname,title in [("straight","roadstate_straight_speed","Straight road"),
                       ("intersection","roadstate_intersection_speed","Intersection")]:
