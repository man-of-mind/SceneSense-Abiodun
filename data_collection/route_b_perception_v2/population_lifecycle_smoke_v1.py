#!/usr/bin/env python3
"""Bounded disposable integration smoke for the phased walker-replacement lifecycle.

Runs the real sensing rig - three 1280x720 cameras plus the 200,000 PPS radar
through ``PerceptionCollectorV2`` with the fast rasterizer - under a small
managed population, destroys three walkers at separately controlled ticks, and
requires each replacement to progress

    body spawn -> observed route tick -> controller attach -> observed route tick
    -> controller start

while the route's tick owner (``SamplingWorld``) sees a strictly contiguous
CARLA frame sequence and the radar sweep aggregator sees no dropped, duplicate
or out-of-order callback.

This is an integration smoke, not a unit-test suite, and not a Route B loop: the
ego stays parked, the sensing rig is real, and everything written is deleted at
the end.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--replenish-interval-s", type=float, default=2.0)
    parser.add_argument(
        "--kill-ticks", type=int, nargs=3, default=(90, 250, 410),
        help="route ticks at which one managed walker body is destroyed",
    )
    parser.add_argument(
        "--work-dir", type=Path,
        default=Path("/tmp/route_b_population_lifecycle_smoke"),
        help="disposable dataset directory; deleted on exit",
    )
    parser.add_argument(
        "--report-json", type=Path,
        default=HERE / "population_lifecycle_smoke_v1.json",
    )
...
def main(argv: list[str] | None = None) -> int:  # noqa: C901 - one linear smoke
    ...
    report: dict[str, Any] = {
        "schema": "route_b_perception_v2.population_lifecycle_smoke.v1",
        "parameters": {
    ...
    try:
        ...
        next_reconcile_tick = 0
        reconcile_every = max(
            1, int(round(float(args.replenish_interval_s) / WORLD_DELTA_S))
        )
        ...
        for route_tick in range(1, int(args.ticks) + 1):
            ...
            if route_tick >= next_reconcile_tick:
                next_reconcile_tick = route_tick + reconcile_every
        ...
        checks = {
            "three_walkers_destroyed": len(killed) == 3 and all(
                row["destroy_result"] for row in killed),
            "three_replacements_created": len(lifecycle_rows) == 3,
    ...
    report["status"] = (
        "POPULATION_LIFECYCLE_SMOKE_PASSED" if passed
        else "POPULATION_LIFECYCLE_SMOKE_FAILED"
    )
