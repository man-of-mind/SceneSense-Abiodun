...
---
flowchart LR
  %% PARKED 2026-08-19: later helper-recipient Phase-2 design.
  %% It is preserved for resumption and is not the current UE-agent diagram.
  ...
  subgraph PUB["POST-INFERENCE PUBLICATION DECISION"]
    ...
    pub["PUBLICATION ACTION<br/>at publication_decision_at_s<br/>state + covariance + motion model<br/>source/recipient/time/byte provenance"]
  ...
  subgraph ACTUATE["LATER FIXED WARNING-ACTUATION ADAPTER"]
    direction TB
  ...
  subgraph EVAL["SEPARATE EVALUATION PLANE — NEVER POLICY STATE"]
    ...
    cf["Matched no-yield counterfactual recipient trajectory<br/>future-hazard label only"]
    shadow["Shadow unchosen LOCAL/SPLIT outputs<br/>evaluation only"]
    metrics["Paired metrics<br/>warning lead, false/missed warning,<br/>bytes, latency, AoI/uncertainty, tracking"]
    gt --> metrics
    cf --> metrics
    shadow --> metrics
    warn --> metrics
  ...
  parked["PARKED 2026-08-19<br/>resume only after explicit Phase-2 authorization"]
  metrics --> parked
