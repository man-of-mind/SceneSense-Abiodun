#!/usr/bin/env bash
# Run the live CARLA Step-1 frontend over OAI 273PRB with UE T-tracer enabled.
#
# This mirrors the default OAI 10FPS T-tracer diagnostic but uses the wider
# 273PRB RAN config.  It records UE-side NRUE_MAC_DCI_GRANT traces, extracts
# 1-second grant windows, and prepares compact artifacts for presentation plots.
set -uo pipefail
...
CONDITION="${CONDITION:-oai_bw273_mu1_ttracer}"
  ...
  (
    cd "${OAI_RAN_BUILD}" &&
      setsid nohup sudo ./nr-uesoftmodem \
        ...
        "${t_args[@]}" \
        > "${LOG_ROOT}/ue_273_ttracer_stdout.log" 2>&1 &
  )
}
...
run_front() {
  ...
  CONDITION="${CONDITION}" \
  TRANSPORT_LABEL="oai_bw273_mu1_noae_ttracer" \
...
postprocess() {
  ...
  say "regenerating presentation plots"
  "${PY}" downlink_latency_fps/plot_oai_ttracer_metrics.py \
    > "${LOG_ROOT}/plot_oai_ttracer_metrics_stdout.log" 2>&1
}
...
start_ue_273
if ! wait_tunnel; then
  say "gNB log: ${LOG_ROOT}/gnb_273_ttracer_stdout.log"
  say "UE log: ${LOG_ROOT}/ue_273_ttracer_stdout.log"
  exit 1
fi
...
say "===== DONE 273PRB CARLA/T-tracer run ${RUN_GROUP} rc=${FRONT_RC} ====="
