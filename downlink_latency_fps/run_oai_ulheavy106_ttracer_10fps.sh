#!/usr/bin/env bash
# Run the corrected live CARLA Step-1 frontend over OAI 106PRB with the
# UL-heavy 4DL/5UL TDD pattern and UE/gNB T-tracer enabled.
set -uo pipefail
...
CONDITION="${CONDITION:-oai_ulheavy_106_ttracer}"
...
GNB_CONF_ULHEAVY="${GNB_CONF_ULHEAVY:-gnb.sa.band78.fr1.106PRB.ulheavy.scenesense_rfsim.conf}"
...
start_gnb_106_ulheavy() {
  ...
  say "starting UL-heavy gNB: ${GNB_CONF_ULHEAVY}, min_rxtxtime=${GNB_MIN_RXTXTIME}, ttracer=${ENABLE_SOFTMODEM_TTRACER}, force_ul_mcs=${FORCE_UL_MCS:-adaptive}"
  (
    cd "${OAI_RAN_BUILD}" &&
      setsid nohup sudo "${sudo_env[@]}" ./nr-softmodem \
        -O "${OAI_RAN_CONF}/${GNB_CONF_ULHEAVY}" \
        --gNBs.[0].min_rxtxtime "${GNB_MIN_RXTXTIME}" \
        --rfsim \
        "${t_args[@]}" \
        > "${LOG_ROOT}/gnb_106_ulheavy_ttracer_stdout.log" 2>&1 &
  )
}
  ...
  (
    cd "${OAI_RAN_BUILD}" &&
      setsid nohup sudo ./nr-uesoftmodem \
        ...
        "${t_args[@]}" \
        > "${LOG_ROOT}/ue_106_ulheavy_ttracer_stdout.log" 2>&1 &
  )
}
...
run_front() {
  ...
  CONDITION="${CONDITION}" \
  TRANSPORT_LABEL="oai_ulheavy_106_noae_ttracer" \
...
postprocess() {
  ...
  printf "Validated corrected-scene 106PRB UL-heavy CARLA/T-tracer run.\n\ngNB config: %s\nUE launch: -r %s -C %s\nRun group: %s\n" \
    "${GNB_CONF_ULHEAVY}" "${UE_PRB}" "${UE_DL_FREQ}" "${RUN_GROUP}" \
    > "${CAP_ROOT}/VALIDATED_106ULHEAVY_TTRACER.ok"
...
start_ue_106
if ! wait_tunnel; then
  say "gNB log: ${LOG_ROOT}/gnb_106_ulheavy_ttracer_stdout.log"
  say "UE log: ${LOG_ROOT}/ue_106_ulheavy_ttracer_stdout.log"
  exit 1
fi
...
say "===== DONE 106PRB UL-heavy CARLA/T-tracer run ${RUN_GROUP} rc=${FRONT_RC} ====="
