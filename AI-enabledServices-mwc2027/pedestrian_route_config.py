#!/usr/bin/env python3
"""Read and write the shared CARLA ego-pedestrian route format.

The module intentionally has no dependency on :mod:`carla`, so both the
scenario-controller UI and a pedestrian camera client can exchange routes
without requiring an active simulator connection.  Its distinct JSON ``type``
prevents a pedestrian client from accidentally loading an ego-vehicle route.

Schema version 1 is a JSON object with the following core fields::

    {
      "schema_version": 1,
      "type": "carla_ego_pedestrian_route",
      "name": "Town10HD ego pedestrian route",
      ...
      "start": {
        "location": {"x": 70.99, "y": 60.97, "z": 0.5},
      ...
      "end": {
        "location": {"x": 60.0, "y": 48.0, "z": 0.0},
        "rotation": {"pitch": 0.0, "yaw": 180.0, "roll": 0.0}
      },
      "planned_path": [
        {"x": 70.99, "y": 60.97, "z": 0.0}
      ],
      "ui_selection": {}
    }
...
The start transform is the exact walker-actor spawn transform.  Route control
points and ``planned_path`` remain on the navigation surface so augmented-
reality arrows can be rendered as ground decals.  Producers may record the
non-negative Z difference as ``spawn_height_offset_m``; when present it is
validated here and consumers must not apply it to the start transform again.
"""
...
PEDESTRIAN_ROUTE_SCHEMA_VERSION = 1
PEDESTRIAN_ROUTE_CONFIG_TYPE = "carla_ego_pedestrian_route"
PEDESTRIAN_ROUTE_COORDINATE_SYSTEM = "carla_world_left_handed_meters"
MIN_ROUTE_SAMPLING_RESOLUTION_M = 0.05
MAX_ROUTE_SAMPLING_RESOLUTION_M = 100.0
MAX_ROUTE_CONTROL_POINTS = 10000
MAX_ROUTE_GUIDE_POINTS = 200000
MAX_SPAWN_HEIGHT_OFFSET_M = 5.0
MAX_ABSOLUTE_COORDINATE_M = 1000000.0
MAX_ABSOLUTE_ROTATION_DEGREES = 360000.0

# Keep the same compact public API as ``ego_route_config.py`` so route
# producers and consumers can switch helpers without duplicating plumbing.
# The values remain pedestrian-specific, most importantly the strict type.
ROUTE_SCHEMA_VERSION = PEDESTRIAN_ROUTE_SCHEMA_VERSION
ROUTE_CONFIG_TYPE = PEDESTRIAN_ROUTE_CONFIG_TYPE
ROUTE_COORDINATE_SYSTEM = PEDESTRIAN_ROUTE_COORDINATE_SYSTEM


class RouteConfigError(ValueError):
    """Raised when an ego-pedestrian route cannot be read or validated."""
...
def _location(value, field_path):
    ...
    for axis in ("x", "y", "z"):
        ...
        if abs(result[axis]) > MAX_ABSOLUTE_COORDINATE_M:
            _field_error(
                "{}.{}".format(field_path, axis),
                "absolute value must not exceed {:.0f} meters".format(
...
def _rotation(value, field_path):
    ...
    for axis in ("pitch", "yaw", "roll"):
        ...
        if abs(number) > MAX_ABSOLUTE_ROTATION_DEGREES:
            _field_error(
                "{}.{}".format(field_path, axis),
                "absolute value must not exceed {:.0f} degrees".format(
                    MAX_ABSOLUTE_ROTATION_DEGREES
                ),
            )
        # CARLA stores rotations as float32. Canonicalizing before crossing the
        # binding avoids both precision loss and equivalent multi-turn values.
        normalized = math.fmod(number, 360.0)
        if normalized >= 180.0:
            normalized -= 360.0
...
def validate_route_config(data):
    """Validate an ego-pedestrian route and return a detached dictionary.
    ...
    # Reject oversized routing arrays before the generic metadata copy walks
    # and duplicates every element.
    raw_intermediates = source.get("intermediate_waypoints")
    if (
        isinstance(raw_intermediates, list)
        and len(raw_intermediates) + 2 > MAX_ROUTE_CONTROL_POINTS
    ):
        _field_error(
            "intermediate_waypoints",
            "route exceeds the {}-control-point limit".format(
                MAX_ROUTE_CONTROL_POINTS
            ),
        )
    raw_planned_path = source.get("planned_path")
    if (
        isinstance(raw_planned_path, list)
        and len(raw_planned_path) > MAX_ROUTE_GUIDE_POINTS
    ):
        _field_error(
            "planned_path",
            "exceeds the {}-point visual-guide limit".format(
    ...
    if not (
        ...
        _field_error(
            ...
            "must be between {:.2f} and {:.1f} meters".format(
                MIN_ROUTE_SAMPLING_RESOLUTION_M,
                MAX_ROUTE_SAMPLING_RESOLUTION_M,
            ),
        )
    ...
    if "spawn_height_offset_m" in source:
        spawn_height_offset = _finite_number(
            source["spawn_height_offset_m"], "spawn_height_offset_m"
        )
        if not 0.0 <= spawn_height_offset <= MAX_SPAWN_HEIGHT_OFFSET_M:
            _field_error(
                "spawn_height_offset_m",
                "must be between 0 and {:.1f} meters".format(
                    MAX_SPAWN_HEIGHT_OFFSET_M
                ),
            )
        result["spawn_height_offset_m"] = spawn_height_offset
    ...
    if (
        isinstance(source["intermediate_waypoints"], list)
        and len(source["intermediate_waypoints"]) + 2
        > MAX_ROUTE_CONTROL_POINTS
    ):
        _field_error(
            "intermediate_waypoints",
    ...
    if "planned_path" in source:
        if (
            isinstance(source["planned_path"], list)
            and len(source["planned_path"]) > MAX_ROUTE_GUIDE_POINTS
        ):
            _field_error(
                "planned_path",
                "exceeds the {}-point visual-guide limit".format(
...
def load_route_config(path):
    """Load *path* as UTF-8 JSON and return a validated pedestrian route."""
...
def save_route_config(path, data, overwrite=True):
    """Validate and atomically save a pedestrian route as UTF-8 JSON.

    The temporary file is created in the destination directory, flushed and
    synced, and then committed atomically, preventing readers from observing a
    partially written route file.  The backward-compatible default replaces an
    existing destination.  With ``overwrite=False``, a same-filesystem hard
    link publishes the temporary file only if the destination does not already
    exist, closing the usual check-then-replace race.
    """
    ...
    try:
        ...
        if overwrite:
            ...
            # Both paths are in destination_dir, so link(2) is an atomic
            # no-clobber commit on the same filesystem.  It also refuses a
            # dangling symlink or any other existing directory entry.
            os.link(temporary_path, route_path)
