...
Version 13 represents spatial RGB-camera/radar sites entirely as virtual
top-down-map glyphs.  No spatial camera or radar actor is spawned, discovered,
started, or subscribed.  Co-located virtual pairs are derived directly from
the ego pedestrian, strictly role-labelled ``spawn_blocker_v5.py`` vehicles
and pedestrians, and nearby live traffic-light roots.  Vehicle and pedestrian
pairs share one front mount computed from their host bounding box; traffic-
light pairs are displayed 15 m above their root.  Complete pairs are
highlighted under normal coverage and only radars are highlighted in either
network-degradation zone.  All other virtual markers remain inactive.  The
existing head RGB camera remains the sole streaming camera because it supplies
this application's rendered view.  Version 13 ignores the optional sensor-
stream bit in the shared ``spawn_blocker_v5.py`` profile while continuing to
consume both zone locations.  Metrics retain the smooth in-zone degradation
model from v12.

Version 12 consumes the world-scoped network-degradation profile published by
``spawn_blocker_v5.py``.  Its top-down view now inventories co-located RGB and
radar sensors on infrastructure, vehicles, and pedestrians, shows a nearest
complete-pair subset as active under normal coverage, and switches to a
radar-only subset inside either degradation zone.  Display activity remains
purely visual by default.  In v12, a shared-profile opt-in could subscribe
allowlisted blocker sensors selected by the map; v13 deliberately removes that
behavior.  Network strength uses the strongest overlapping radial
smoothstep and a small spatial hysteresis prevents modality chatter at zone
boundaries.  Local zone overrides remain available for isolated testing.

Version 11 highlights the occluded hazard actors owned by
``spawn_blocker_v5.py`` inside the ego-pedestrian spatial map. Reactive
vehicles and blocker pedestrians use red markers with adjacent ``OCCLUDED
VEHICLE`` or ``OCCLUDED PEDESTRIAN`` labels, and blocker-pedestrian circles
are enlarged for rapid comprehension. Static occluder vehicles and ordinary
traffic retain their existing colors. Role-based classification is repeated
on every map refresh, so replacement actor IDs require no special handling.

Version 10 adds a proximity-aware AR warning for the occluded reactive vehicle
spawned by ``spawn_blocker_v5.py``. While bounding-box visualizations are on,
the reactive vehicle changes from blue to red inside a configurable radius,
receives a ``VEHICLE`` class label, and activates a top-right ``APPROACHING
VEHICLE / STOP`` warning card. The alert clears after the vehicle passes the
ego pedestrian. Version 9's route arrows, top-down map, and live metrics remain
grouped under U.
...
--------
    ...
    U                   toggle boxes/vehicle alert, route, map, and metrics
    Space               jump
    Esc / Q             quit
...
-------
    ...
    python3 pedestrian_head_camera_client_v13.py --record-route recorded_route.json

    # Re-run from the recorded start and press U to show its waypoint arrows.
    python3 pedestrian_head_camera_client_v13.py \\
...
# spawn_blocker_v5.py assigns its moving blocker a stable role of the form
# reactive_blocker_v5_<positive index>. Actor IDs are not stable because the
# spawner replaces the vehicle after every encounter.
...
DEFAULT_ROGUE_VEHICLE_WARNING_RADIUS_M = 45.0
ROGUE_VEHICLE_BOX_COLOR = (255, 35, 35)
ROGUE_VEHICLE_WARNING_COLOR = (255, 48, 48)
ROGUE_VEHICLE_LABEL_BACKGROUND = (160, 18, 18)
ROGUE_VEHICLE_LABEL = "VEHICLE"
ROGUE_VEHICLE_WARNING_TITLE = "APPROACHING VEHICLE"
ROGUE_VEHICLE_WARNING_ACTION = "STOP"
ROGUE_VEHICLE_PASS_MARGIN_M = 1.0
ROGUE_VEHICLE_PASS_EXTRA_CLEARANCE_M = 0.25
ROGUE_VEHICLE_SPEED_DIRECTION_THRESHOLD_MPS = 0.25
ROGUE_VEHICLE_DISTANCE_EPSILON_M = 1.0e-4
ROGUE_VEHICLE_WARNING_CARD_WIDTH = 420
ROGUE_VEHICLE_WARNING_CARD_HEIGHT = 108
...
# World-profile-driven network degradation and spatial-sensor visualization.
# A valid world profile is authoritative unless an explicit local override is
# supplied.  No profile means no implicit degradation, preserving truthful
# behavior when spawn_blocker_v5.py is not running.
...
NETWORK_POLICY_EXIT_HYSTERESIS_M = 0.50
...
DEFAULT_SPATIAL_MAP_DEGRADED_RADARS = 5
MAX_SPATIAL_MAP_ACTIVE_SENSORS = 1000
SENSOR_SITE_QUANTIZATION_PER_METER = 10.0
HEAD_RENDER_CAMERA_ROLE = "pedestrian_head_v13_render_camera"
VIRTUAL_SENSOR_FRONT_MARGIN_M = 0.05
VIRTUAL_VEHICLE_SENSOR_FALLBACK_X_M = 2.50
VIRTUAL_VEHICLE_SENSOR_FALLBACK_Z_M = 1.00
VIRTUAL_PEDESTRIAN_SENSOR_FALLBACK_X_M = 0.35
VIRTUAL_PEDESTRIAN_SENSOR_FALLBACK_Z_M = 1.55
VIRTUAL_PEDESTRIAN_SENSOR_MIN_Z_M = 1.45
VIRTUAL_PEDESTRIAN_SENSOR_MAX_Z_M = 2.00
VIRTUAL_VEHICLE_SENSOR_MIN_Z_M = 0.55
VIRTUAL_TRAFFIC_LIGHT_SENSOR_HEIGHT_M = 15.0

# v13 keeps spatial-map activity virtual and visual-only. The head-render
# camera is the only RGB/radar-type sensor this client spawns or subscribes,
# regardless of a shared-profile opt-in.
SPATIAL_MAP_SENSOR_STREAMING_PERMITTED = False
...
def nonnegative_int(value: str) -> int:
    """Parse a bounded-count candidate; the upper bound is checked later."""
    try:
...
def normalize_network_degradation_zones(zones) -> Tuple[Tuple[float, float, float], ...]:
    """Accept profile dataclasses or plain triples and return finite triples."""
    normalized = []
    for zone in zones or ():
        if hasattr(zone, "as_tuple"):
            values = zone.as_tuple()
        elif isinstance(zone, dict):
            values = (zone.get("x"), zone.get("y"), zone.get("radius"))
        else:
            values = zone
        try:
            if len(values) != 3:
                raise ValueError("each network degradation zone needs X Y RADIUS")
            x_coord, y_coord, radius = [float(value) for value in values]
        except (TypeError, ValueError, OverflowError) as exc:
            raise ValueError(
                "each network degradation zone needs finite X Y RADIUS values"
            ) from exc
...
def network_degradation_strength(location, zones) -> float:
    """Return the strongest smooth radial influence, never an overlap sum."""
    try:
...
def network_radar_policy_active(location, zones, previously_active: bool) -> bool:
    """Latch radar priority across a narrow outer band to avoid edge chatter."""
    try:
        x_coord = float(location.x)
        y_coord = float(location.y)
    except (AttributeError, TypeError, ValueError, OverflowError):
        return False
    if not zones or not math.isfinite(x_coord) or not math.isfinite(y_coord):
        return False
    margin = NETWORK_POLICY_EXIT_HYSTERESIS_M if previously_active else 0.0
    return any(
        math.hypot(x_coord - zone_x, y_coord - zone_y) < radius + margin
        for zone_x, zone_y, radius in zones
    )
...
def rogue_vehicle_role_prefix(value: str) -> str:
    """Normalize the role stem used for reactive blocker matching."""
...
def actor_has_indexed_role(
    actor,
    type_prefix: str,
    role_prefix: str,
) -> bool:
    """Match ``TYPE`` actors whose role is exactly ``PREFIX_<positive int>``."""
    try:
    ...
    if not type_id.startswith(str(type_prefix)):
        return False
    indexed_prefix = "{}_".format(str(role_prefix).rstrip("_"))
...
def actor_is_rogue_vehicle(actor, role_prefix: str) -> bool:
    """Match only indexed reactive vehicles owned by spawn_blocker_v5."""
    return actor_has_indexed_role(actor, "vehicle.", role_prefix)


def actor_is_spawn_blocker_vehicle(
    actor,
    reactive_role_prefix: str = DEFAULT_ROGUE_VEHICLE_ROLE_PREFIX,
    static_role_prefix: str = DEFAULT_STATIC_BLOCKER_VEHICLE_ROLE_PREFIX,
) -> bool:
    """Match indexed static or reactive vehicles owned by spawn_blocker_v5."""
    return actor_has_indexed_role(
        actor,
        "vehicle.",
        static_role_prefix,
    ) or actor_has_indexed_role(
        actor,
        "vehicle.",
        reactive_role_prefix,
    )


def actor_is_rogue_pedestrian(
    actor,
    role_prefix: str = DEFAULT_ROGUE_PEDESTRIAN_ROLE_PREFIX,
) -> bool:
    """Match only indexed blocker walkers owned by spawn_blocker_v5."""
    return actor_has_indexed_role(actor, "walker.pedestrian.", role_prefix)


def actor_planar_bounding_radius(actor) -> float:
    """Return a conservative XY bounding support without issuing an RPC."""
    try:
...
def rogue_vehicle_alert_state(
    ego_transform: carla.Transform,
    vehicle_transform: carla.Transform,
    vehicle_velocity: carla.Vector3D,
    warning_radius: float,
    pass_clearance: float,
) -> Tuple[Optional[float], bool]:
    """Return (distance, passed) for one frame-matched reactive vehicle."""
    ego_location = ego_transform.location
    vehicle_location = vehicle_transform.location
    delta_x = float(ego_location.x - vehicle_location.x)
    delta_y = float(ego_location.y - vehicle_location.y)
    distance = math.hypot(delta_x, delta_y)
    if (
        not math.isfinite(distance)
        or distance
        > float(warning_radius) + ROGUE_VEHICLE_DISTANCE_EPSILON_M
    ):
        return None, False

    velocity_x = float(vehicle_velocity.x)
    velocity_y = float(vehicle_velocity.y)
    planar_speed = math.hypot(velocity_x, velocity_y)
    if (
        math.isfinite(planar_speed)
        and planar_speed > ROGUE_VEHICLE_SPEED_DIRECTION_THRESHOLD_MPS
    ):
        forward_x = velocity_x / planar_speed
        forward_y = velocity_y / planar_speed
    else:
    ...
    longitudinal_distance = delta_x * forward_x + delta_y * forward_y
    passed = (
        math.isfinite(longitudinal_distance)
        and longitudinal_distance < -float(pass_clearance)
    )
    if passed:
        return None, True
    return distance, False
...
def project_actor_box(
    ...
    """Project an actor box with near-plane edge clipping."""
    try:
    ...
    # CARLA camera coordinates are x forward, y right, z up. Keep every front
    # corner and clip all 12 box edges to the near plane. Discarding rear
    # corners makes a close vehicle's box collapse immediately before impact.
    near_plane = 0.10
    bbox_edges = (
        (0, 1), (0, 2), (0, 4),
        (1, 3), (1, 5),
    ...
    clipped_points = [
        point for point in camera_points if float(point[0]) >= near_plane
    ]
    for first_index, second_index in bbox_edges:
        first = camera_points[first_index]
        second = camera_points[second_index]
    ...
    camera_points = np.asarray(clipped_points, dtype=np.float64)
    ...
    finite_pixels = np.isfinite(horizontal) & np.isfinite(vertical)
    if not np.any(finite_pixels):
        return None
    horizontal = horizontal[finite_pixels]
    vertical = vertical[finite_pixels]
    depths = depths[finite_pixels]
...
def draw_ground_truth_boxes(
    ...
    alert_origin_actor: Optional[carla.Actor] = None,
    rogue_vehicle_role: str = DEFAULT_ROGUE_VEHICLE_ROLE_PREFIX,
    rogue_vehicle_warning_radius: float = DEFAULT_ROGUE_VEHICLE_WARNING_RADIUS_M,
    rogue_vehicle_label: Optional[pygame.Surface] = None,
    passed_rogue_vehicle_ids: Optional[set] = None,
) -> Tuple[int, int, Optional[float]]:
    """Draw GT overlays and return counts plus nearest rogue alert distance."""
    ...
    projected = []
    nearest_rogue_distance = None
    active_rogue_vehicle_ids = set()
    alert_origin_transform = None
    alert_origin_radius = 0.0
    if alert_origin_actor is not None:
        try:
            alert_origin_snapshot = snapshot.find(int(alert_origin_actor.id))
            if alert_origin_snapshot is not None:
                alert_origin_transform = alert_origin_snapshot.get_transform()
                alert_origin_radius = actor_planar_bounding_radius(
                    alert_origin_actor
                )
        except (AttributeError, RuntimeError, TypeError, ValueError):
            alert_origin_transform = None
    ...
    for actor in actors:
        try:
            ...
            if actor_id in excluded:
                continue
            is_rogue_vehicle = actor_is_rogue_vehicle(
                actor, rogue_vehicle_role
            )
            if is_rogue_vehicle:
                active_rogue_vehicle_ids.add(actor_id)
            actor_snapshot = snapshot.find(actor_id)
            if actor_snapshot is None:
                continue
            ...
            if (
                is_rogue_vehicle
                and alert_origin_transform is not None
                and (
                    passed_rogue_vehicle_ids is None
                    or actor_id not in passed_rogue_vehicle_ids
                )
            ):
                try:
                    vehicle_velocity = actor_snapshot.get_velocity()
                except (AttributeError, RuntimeError):
                    vehicle_velocity = carla.Vector3D()
                pass_clearance = max(
                    ROGUE_VEHICLE_PASS_MARGIN_M,
                    alert_origin_radius
                    + actor_planar_bounding_radius(actor)
                    + ROGUE_VEHICLE_PASS_EXTRA_CLEARANCE_M,
                )
                alert_distance, vehicle_has_passed = rogue_vehicle_alert_state(
                    alert_origin_transform,
                    actor_transform,
                    vehicle_velocity,
                    rogue_vehicle_warning_radius,
                    pass_clearance,
                )
                if vehicle_has_passed and passed_rogue_vehicle_ids is not None:
                    passed_rogue_vehicle_ids.add(actor_id)
                if (
                    alert_distance is not None
                    and (
                        nearest_rogue_distance is None
                        or alert_distance < nearest_rogue_distance
                    )
                ):
                    # Proximity is intentionally evaluated before projection:
                    # an occluder or a turned-away head camera must not hide
                    # the top-right safety warning.
                    nearest_rogue_distance = alert_distance

            camera_distance = actor_transform.location.distance(camera_location)
            if camera_distance > max_distance and alert_distance is None:
                continue
        ...
        projected.append(
            (
                ...
                actor_id,
                is_pedestrian,
                bool(is_rogue_vehicle and alert_distance is not None),
            )
        )

    if passed_rogue_vehicle_ids is not None:
        # A respawn receives a new ID and is automatically re-armed. Pruning
        # retired IDs also bounds this set across arbitrarily many encounters.
        passed_rogue_vehicle_ids.intersection_update(active_rogue_vehicle_ids)

    # Draw distant boxes first so nearer outlines and labels remain legible.
    projected.sort(key=lambda item: item[0], reverse=True)
    vehicle_count = 0
    pedestrian_count = 0
    for depth, rect, actor_id, is_pedestrian, is_rogue_alert in projected:
        if is_pedestrian:
            ...
            color = (
                ROGUE_VEHICLE_BOX_COLOR
                if is_rogue_alert else VEHICLE_BOX_COLOR
            )
            label_kind = "VEH"
            vehicle_count += 1

        pygame.draw.rect(surface, color, rect, 3 if is_rogue_alert else 2)
        if is_rogue_alert and rogue_vehicle_label is not None:
            label = rogue_vehicle_label
        else:
            label = font.render(
        ...
        if is_rogue_alert:
            ...
            pygame.draw.rect(
                surface,
                ROGUE_VEHICLE_LABEL_BACKGROUND,
                label_background,
                border_radius=3,
            )
        surface.blit(label, (label_x, label_y))

    return vehicle_count, pedestrian_count, nearest_rogue_distance
...
class LiveMetricsModel:
    ...
    def __init__(
        self,
        placeholder_seed: int = DEFAULT_METRICS_PLACEHOLDER_SEED,
        network_map_latency_penalty_ms: float = (
            DEFAULT_NETWORK_MAP_LATENCY_PENALTY_MS
        ),
        network_map_accuracy_penalty_cm: float = (
            DEFAULT_NETWORK_MAP_ACCURACY_PENALTY_CM
        ),
        network_sense_to_act_penalty_ms: float = (
            DEFAULT_NETWORK_SENSE_TO_ACT_PENALTY_MS
        ),
    ):
        self._rng = random.Random(int(placeholder_seed))
        self._network_map_latency_penalty_ms = max(
            0.0, float(network_map_latency_penalty_ms)
        )
        self._network_map_accuracy_penalty_cm = max(
            0.0, float(network_map_accuracy_penalty_cm)
        )
        self._network_sense_to_act_penalty_ms = max(
            0.0, float(network_sense_to_act_penalty_ms)
        )
    ...
    def snapshot(
        ...
        if network_sensor_policy not in ("BALANCED", "RADAR PRIORITY"):
            network_sensor_policy = (
                "RADAR PRIORITY" if degradation_strength > 0.0 else "BALANCED"
            )
        ...
        return {
            ...
            "spatial_map_accuracy_cm": spatial_accuracy_cm,
            ...
            "network_stream_sensors": bool(network_stream_sensors),
        }
...
class LiveMetricsRenderer:
    ...
    def build_frame(self, metrics: Dict[str, object]) -> np.ndarray:
        ...
        network_sensor_policy = str(
            metrics.get("network_sensor_policy", "BALANCED")
        )
        if network_sensor_policy not in ("BALANCED", "RADAR PRIORITY"):
            network_sensor_policy = (
                "RADAR PRIORITY" if network_degraded else "BALANCED"
            )
        ...
        network_header = (
            "DEGRADED {:3.0f}% | {} | DISPLAY ONLY".format(
                degradation_strength * 100.0,
                network_sensor_policy,
            )
            if network_degraded
            else "RECOVERY HYSTERESIS | RADAR PRIORITY | DISPLAY ONLY"
            if network_sensor_policy == "RADAR PRIORITY"
            else "NORMAL | BALANCED | DISPLAY ONLY"
        )
        cv2.putText(
            frame,
            "LIVE EGO-PEDESTRIAN PHYSICAL AI METRICS | " + network_header,
            (14, 18),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.48,
            (220, 224, 230),
...
class SpatialSensorSubscriptionController:
    """Compatibility guard: virtual map sites can never be subscribed."""

    def __init__(self) -> None:
        self._owned = {}

    def reconcile(self, selected_sensors, enabled: bool) -> None:
        """Ignore selections; virtual sites have no corresponding Sensor."""
        if enabled:
            LOG.error(
                "Internal request attempted to enable spatial streams, but "
                "v13 is display-only; request ignored"
            )
        self._owned.clear()
    ...
    @property
    def owned_actor_ids(self) -> Tuple[int, ...]:
        return tuple(sorted(self._owned))

    def receipt_scalars(self):
        return {}
...
class TopDownMapRenderer:
    ...
    def __init__(
        ...
        rogue_vehicle_role_prefix: str = DEFAULT_ROGUE_VEHICLE_ROLE_PREFIX,
        rogue_pedestrian_role_prefix: str = (
            DEFAULT_ROGUE_PEDESTRIAN_ROLE_PREFIX
        ),
        local_network_zones=None,
        disable_network_degradation: bool = False,
        active_sensor_pairs: int = DEFAULT_SPATIAL_MAP_ACTIVE_SENSOR_PAIRS,
        degraded_active_radars: int = DEFAULT_SPATIAL_MAP_DEGRADED_RADARS,
    ) -> None:
        ...
        self._local_network_zones = (
            None
            if local_network_zones is None
            else normalize_network_degradation_zones(local_network_zones)
        )
        self._disable_network_degradation = bool(disable_network_degradation)
        self._active_sensor_pairs = int(active_sensor_pairs)
        self._degraded_active_radars = int(degraded_active_radars)
        if not 0 <= self._active_sensor_pairs <= MAX_SPATIAL_MAP_ACTIVE_SENSORS:
            raise ValueError("spatial-map active sensor-pair count is out of range")
        if not 0 <= self._degraded_active_radars <= MAX_SPATIAL_MAP_ACTIVE_SENSORS:
            raise ValueError("degraded active-radar count is out of range")
        self.last_network_degradation_zones = ()
        self.last_network_degradation_strength = 0.0
        self.last_network_radar_priority = False
        self.last_profile_stream_sensors = False
        self.last_profile_stream_requested = False
        self.last_profile_token = None
        self.last_selected_stream_sensors = {}
        self.last_streaming_sensor_count = 0
        self._last_profile_error = None
    ...
    def _refresh_network_profile(self) -> None:
        """Resolve one committed world profile; partial manifests fail closed."""
        profile = None
        try:
            ...
            error_text = str(exc)
            if error_text != self._last_profile_error:
                LOG.warning(
                    "Ignoring invalid network-degradation world profile: %s",
                    exc,
                )
            self._last_profile_error = error_text
        else:
            self._last_profile_error = None

        profile_zones = () if profile is None else profile.zone_tuples
        if self._disable_network_degradation:
            zones = ()
        elif self._local_network_zones is not None:
            zones = self._local_network_zones
        else:
            zones = profile_zones
        zones = normalize_network_degradation_zones(zones)
        token = None if profile is None else profile.session_token
        stream_requested = bool(
            profile is not None and profile.start_active_sensors
        )
        stream_sensors = bool(
            SPATIAL_MAP_SENSOR_STREAMING_PERMITTED and stream_requested
        )
        state_changed = (
            zones != self.last_network_degradation_zones
            or token != self.last_profile_token
            or stream_sensors != self.last_profile_stream_sensors
            or stream_requested != self.last_profile_stream_requested
        )
        self.last_network_degradation_zones = zones
        self.last_profile_token = token
        self.last_profile_stream_sensors = stream_sensors
        self.last_profile_stream_requested = stream_requested
        if state_changed:
            LOG.info(
                "Network profile: token=%s zones=%s spatial_streams=%s%s%s",
                token or "none",
                ", ".join(
                    "({:.3f},{:.3f},r={:.2f})".format(*zone)
                    for zone in zones
                ) or "none",
                stream_sensors,
                " (profile stream request ignored by display-only v13)"
                if stream_requested
                else "",
                " (local zone override)"
                if self._local_network_zones is not None
                else "",
            )
    ...
    def _draw_network_degradation_zones(self, image: np.ndarray) -> None:
        """Draw transparent circles beneath actors and sensor markers."""
        visible_bounds = self._visible_world_bounds()
        for zone_x, zone_y, radius in self.last_network_degradation_zones:
            zone_bounds = (
                zone_x - radius,
                zone_y - radius,
                zone_x + radius,
    ...
    @staticmethod
    def _virtual_sensor_actor_id(parent_id: int, modality: str) -> int:
        """Return a stable negative display ID with no CARLA actor meaning."""
        parent_id = abs(int(parent_id))
        modality_offset = 1 if modality == "camera" else 2
        return -(2 * parent_id + modality_offset)
    ...
    def _virtual_sensor_pair(
        ...
        """Build one co-located camera/radar pair made only of map metadata."""
        values = (
            float(x_coord),
            float(y_coord),
            float(z_coord),
        ...
        markers = []
        for modality in ("camera", "radar"):
            markers.append(
                {
                    "actor_id": cls._virtual_sensor_actor_id(
                        parent_id,
                        modality,
                    ),
                    "sensor": None,
                    "type_id": "virtual.sensor.{}".format(modality),
    ...
    @classmethod
    def _virtual_front_sensor_pair(
        cls,
        actor,
        actor_transform: carla.Transform,
        host_category: str,
    ):
        """Derive one virtual front mount from an upright host actor."""
        try:
            actor_id = int(actor.id)
            type_id = str(actor.type_id)
            is_pedestrian = type_id.startswith("walker.pedestrian.")
            location = actor_transform.location
            yaw_degrees = float(actor_transform.rotation.yaw)
            fallback_x = (
                VIRTUAL_PEDESTRIAN_SENSOR_FALLBACK_X_M
                if is_pedestrian
                else VIRTUAL_VEHICLE_SENSOR_FALLBACK_X_M
            )
            fallback_z = (
                VIRTUAL_PEDESTRIAN_SENSOR_FALLBACK_Z_M
                if is_pedestrian
                else VIRTUAL_VEHICLE_SENSOR_FALLBACK_Z_M
            )
            local_x = fallback_x
            local_y = 0.0
            local_z = fallback_z
            try:
                ...
                if not all(math.isfinite(value) for value in box_values):
                    raise ValueError("non-finite host bounding box")
                local_x = (
                    box_values[0]
                    + max(0.0, box_values[3])
                    + VIRTUAL_SENSOR_FRONT_MARGIN_M
                )
                local_y = box_values[1]
                center_upper_z = box_values[2] + 0.5 * max(
                    0.0,
                    box_values[4],
                )
                if is_pedestrian:
                    local_z = max(
                        VIRTUAL_PEDESTRIAN_SENSOR_MIN_Z_M,
                        min(
                            VIRTUAL_PEDESTRIAN_SENSOR_MAX_Z_M,
                            center_upper_z,
                        ),
                    )
                else:
                    local_z = max(
                        VIRTUAL_VEHICLE_SENSOR_MIN_Z_M,
                        center_upper_z,
                    )
            except (AttributeError, RuntimeError, TypeError, ValueError):
                pass
            ...
            world_x = float(location.x) + cosine * local_x - sine * local_y
            world_y = float(location.y) + sine * local_x + cosine * local_y
            world_z = float(location.z) + local_z
            return cls._virtual_sensor_pair(
                actor_id,
                type_id,
                world_x,
                world_y,
    ...
    @classmethod
    def _virtual_traffic_light_sensor_pair(
        cls,
        traffic_light,
        traffic_light_transform: carla.Transform,
    ):
        """Place one virtual pair 15 m above a live traffic-light root."""
        try:
            actor_id = int(traffic_light.id)
            type_id = str(traffic_light.type_id)
            location = traffic_light_transform.location
            return cls._virtual_sensor_pair(
                actor_id,
                type_id,
                float(location.x),
                float(location.y),
                float(location.z) + VIRTUAL_TRAFFIC_LIGHT_SENSOR_HEIGHT_M,
                float(traffic_light_transform.rotation.yaw),
                "traffic_light",
            )
        except (AttributeError, RuntimeError, TypeError, ValueError):
            return ()
    ...
    @staticmethod
    def _sensor_site_key(marker):
        location_key = (
            int(round(float(marker["x"]) * SENSOR_SITE_QUANTIZATION_PER_METER)),
            int(round(float(marker["y"]) * SENSOR_SITE_QUANTIZATION_PER_METER)),
            int(round(float(marker["z"]) * SENSOR_SITE_QUANTIZATION_PER_METER)),
        )
    ...
    def _prepare_sensor_markers(self, virtual_markers):
        """Apply the visual selection policy to actor-derived virtual sites."""
        # Copy and deduplicate defensively so policy mutation never leaks into
        # the actor traversal that generated this refresh's inventory.
        deduplicated = {}
        for source_marker in virtual_markers:
            try:
                marker = source_marker.copy()
                if marker["modality"] not in ("camera", "radar"):
                    continue
                if not self._location_is_visible(
                    carla.Location(
                        x=float(marker["x"]),
                        y=float(marker["y"]),
                        z=float(marker["z"]),
                    ...
                    continue
                key = (marker["modality"], self._sensor_site_key(marker))
                current = deduplicated.get(key)
                if current is None or int(marker["actor_id"]) < int(
                    current["actor_id"]
                ):
                    deduplicated[key] = marker
            except (KeyError, TypeError, ValueError):
                continue
        markers = list(deduplicated.values())

        sites = {}
        for marker in markers:
            marker["active"] = False
            sites.setdefault(self._sensor_site_key(marker), []).append(marker)
        ...
        def site_sort_key(item):
            ...
            minimum_actor_id = min(
                int(marker["actor_id"]) for marker in site_markers
            )
            return minimum_distance, repr(key), minimum_actor_id

        ordered_sites = sorted(sites.items(), key=site_sort_key)
        for _key, site_markers in ordered_sites:
            modalities = {marker["modality"] for marker in site_markers}
            if {"camera", "radar"}.issubset(modalities):
                for marker in site_markers:
                    marker["pixel_offset"] = (
                        (-8, 0) if marker["modality"] == "camera" else (8, 0)
                    )

        # "Active" is strictly a visual policy over virtual metadata. There is
        # no corresponding CARLA Sensor actor or listen state.
        if self.last_network_radar_priority:
            selected = 0
            for _key, site_markers in ordered_sites:
                radars_at_site = [
                    marker
                    for marker in site_markers
                    if marker["modality"] == "radar"
                ]
                if not radars_at_site:
                    continue
                if selected >= self._degraded_active_radars:
                    break
                min(radars_at_site, key=lambda marker: marker["actor_id"])[
                    "active"
                ] = True
                selected += 1
        else:
            selected = 0
            for _key, site_markers in ordered_sites:
                selectable_markers = site_markers
                modalities = {
                    marker["modality"] for marker in selectable_markers
                }
                if not {"camera", "radar"}.issubset(modalities):
                    continue
                if selected >= self._active_sensor_pairs:
                    break
                for marker in selectable_markers:
                    if marker["modality"] in ("camera", "radar"):
                        marker["active"] = True
                selected += 1
    ...
    def _draw_sensor_marker(self, image, marker) -> None:
        center_x, center_y = self._world_xy_to_pixel(marker["x"], marker["y"])
        ...
        forward = np.asarray(
            (math.cos(yaw_radians), math.sin(yaw_radians)), dtype=np.float32
        )
        ...
        if marker["modality"] == "camera":
            color = (
                ...
                if active
                else TOPDOWN_COLOR_CAMERA_INACTIVE
            )
        ...
        color = (
            ...
            if active and self.last_network_radar_priority
            else TOPDOWN_COLOR_RADAR_ACTIVE
            if active
            else TOPDOWN_COLOR_RADAR_INACTIVE
        )
        diamond = np.asarray(
            (
                (center[0], center[1] - 8),
        ...
        if active and self.last_network_radar_priority:
            cv2.circle(image, center, 15, color, 2, lineType=cv2.LINE_AA)
        if active:
    ...
    def _draw_sensors(self, image, virtual_markers):
        markers = self._prepare_sensor_markers(virtual_markers)
        counts = {
            "camera_active": 0,
            "camera_inactive": 0,
            "radar_active": 0,
        ...
        for marker in markers:
            ...
            if not self._location_is_visible(location):
                continue
            self._draw_sensor_marker(image, marker)
            counts["{}_{}".format(
                marker["modality"],
                "active" if marker["active"] else "inactive",
            )] += 1
        return counts
    ...
    @staticmethod
    def _label_overlap_area(first, second) -> int:
    ...
    def _draw_live_actors(
        ...
        occluded_labels = []
        virtual_sensor_markers = list(
            self._virtual_front_sensor_pair(
                hero_actor,
                hero_transform,
                "ego_pedestrian",
            )
        ...
        for vehicle in vehicles:
            ...
            try:
                ...
                if actor_is_spawn_blocker_vehicle(
                    vehicle,
                    reactive_role_prefix=self._rogue_vehicle_role_prefix,
                ):
                    virtual_sensor_markers.extend(
                        self._virtual_front_sensor_pair(
                            vehicle,
                            actor_transform,
                            "blocker_vehicle",
                ...
                is_occluded_vehicle = actor_is_rogue_vehicle(
                    vehicle,
                    self._rogue_vehicle_role_prefix,
                )
                self._draw_vehicle(
                    image,
                    vehicle,
        ...
        for pedestrian in pedestrians:
            ...
            try:
                ...
                if actor_is_rogue_pedestrian(
                    pedestrian,
                    self._rogue_pedestrian_role_prefix,
                ):
                    virtual_sensor_markers.extend(
                        self._virtual_front_sensor_pair(
                            pedestrian,
        ...
        for traffic_light in traffic_lights:
            try:
                ...
                virtual_sensor_markers.extend(
                    self._virtual_traffic_light_sensor_pair(
                        traffic_light,
    ...
    def _draw_status(
        ...
        degradation_strength = self.last_network_degradation_strength
        stream_status = "VIRTUAL SENSORS DISPLAY ONLY"
        if degradation_strength > 0.0:
            network_status = (
                "CELLULAR: DEGRADED {:3.0f}% | RADAR-ONLY | {}".format(
                    degradation_strength * 100.0,
                    stream_status,
                )
            )
        elif self.last_network_radar_priority:
            network_status = "CELLULAR: RECOVERY HYSTERESIS | RADAR-ONLY | {}".format(
                stream_status
            )
        else:
            network_status = "CELLULAR: NORMAL | CAMERA+RADAR SUBSET | {}".format(
                stream_status
            )
        cv2.putText(
            frame,
        ...
        if self.last_network_degradation_zones:
            cv2.circle(
                frame,
            ...
            cv2.putText(
                frame,
                "CELLULAR ZONES {}".format(
                    len(self.last_network_degradation_zones)
                ),
                (x_coord + 15, legend_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.43,
        ...
        sensor_status = (
            "CAM active {camera_active} / inactive {camera_inactive}    "
            "RADAR active {radar_active} / inactive {radar_inactive}"
        ).format(**sensor_counts)
        cv2.putText(
            frame,
            sensor_status,
            (self._plot_left, self._height - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.42,
            (215, 220, 227),
            1,
    ...
    def invalidate_dynamic_selection(self) -> None:
        """Discard a cached ego-relative policy and force the next refresh."""
        self._last_refresh_time = None
        self.last_network_degradation_strength = 0.0
        self.last_network_radar_priority = False
        self.last_selected_stream_sensors = {}
    ...
    def render(
        ...
        try:
            ...
            if subscription_controller is not None:
                subscription_controller.reconcile({}, enabled=False)
                self.last_streaming_sensor_count = len(
                    subscription_controller.owned_actor_ids
                )
            return
        ...
        self.last_network_radar_priority = network_radar_policy_active(
            hero_transform.location,
            self.last_network_degradation_zones,
            self.last_network_radar_priority,
        )
        ...
        if subscription_controller is not None:
            subscription_controller.reconcile(
                self.last_selected_stream_sensors,
                enabled=False,
            )
            self.last_streaming_sensor_count = len(
...
def toggle_topdown_map(
    ...
    if renderer is None:
        try:
            renderer = TopDownMapRenderer(
                world,
                carla_map,
                zoom_radius,
                rogue_vehicle_role_prefix=rogue_vehicle_role_prefix,
                rogue_pedestrian_role_prefix=rogue_pedestrian_role_prefix,
                local_network_zones=local_network_zones,
                disable_network_degradation=disable_network_degradation,
                active_sensor_pairs=active_sensor_pairs,
                degraded_active_radars=degraded_active_radars,
            )
        except Exception as exc:
...
def spawn_rgb_camera(
    ...
    if blueprint.has_attribute("role_name"):
        blueprint.set_attribute("role_name", HEAD_RENDER_CAMERA_ROLE)
...
def build_rogue_vehicle_warning_surface(
    title_font: pygame.font.Font,
    action_font: pygame.font.Font,
) -> pygame.Surface:
    """Build one reusable warning card styled after the supplied reference."""
    card = pygame.Surface(
        (ROGUE_VEHICLE_WARNING_CARD_WIDTH, ROGUE_VEHICLE_WARNING_CARD_HEIGHT),
        pygame.SRCALPHA,
    )
    bounds = card.get_rect()
    pygame.draw.rect(card, (7, 9, 13, 232), bounds, border_radius=12)
    pygame.draw.rect(
        card,
        ROGUE_VEHICLE_WARNING_COLOR,
        bounds,
        2,
        border_radius=12,
    )

    triangle = ((17, 89), (46, 20), (75, 89))
    pygame.draw.polygon(
        card, ROGUE_VEHICLE_WARNING_COLOR, triangle, 3
    )
    exclamation = action_font.render("!", True, (255, 255, 255))
    card.blit(
        exclamation,
        (
            46 - exclamation.get_width() // 2,
            49 - exclamation.get_height() // 2,
        ),
    )

    title_first, title_second = ROGUE_VEHICLE_WARNING_TITLE.split(" ", 1)
    approaching = title_font.render(
        title_first, True, (245, 245, 245)
    )
    vehicle = title_font.render(title_second, True, (245, 245, 245))
    action = action_font.render(
        ROGUE_VEHICLE_WARNING_ACTION,
        True,
        ROGUE_VEHICLE_WARNING_COLOR,
    )
    card.blit(approaching, (91, 10))
    card.blit(vehicle, (91, 36))
    card.blit(action, (91, 69))
    return card


def draw_rogue_vehicle_warning(
    display: pygame.Surface,
    warning_surface: pygame.Surface,
) -> None:
    """Draw the warning top-right and above the full-width HUD."""
    margin = 16
    x_coord = max(0, display.get_width() - warning_surface.get_width() - margin)
    y_coord = min(
        margin,
        max(0, display.get_height() - warning_surface.get_height()),
    )
    display.blit(warning_surface, (x_coord, y_coord))
...
def draw_hud(
    ...
    lines = [
        ...
        "Visuals: B boxes/vehicle alert   U: route {} / map {} ({:.1f} m) / metrics {}".format(
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "repeatable local cellular-degradation circle; explicit local "
            "zones override world-profile zone locations; virtual sensor "
            "markers remain display-only"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "ignore world-profile and local degradation zones; virtual "
            "sensor markers remain display-only"
        ),
    )
    parser.add_argument(
        ...
        metavar="COUNT",
        help=(
            "nearest complete camera/radar sites shown active outside degraded "
            "zones (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--spatial-map-degraded-radars",
        "--spatial-map-degraded-radar-pairs",
        dest="spatial_map_degraded_radars",
        type=nonnegative_int,
        default=DEFAULT_SPATIAL_MAP_DEGRADED_RADARS,
        metavar="COUNT",
        help=(
            "nearest radars shown active while radar priority is latched "
            "(default: %(default)s)"
        ),
    )
    parser.add_argument(
        ...
        metavar="MS",
        help="maximum modeled map-latency addition at a zone center",
    )
    parser.add_argument(
        ...
        metavar="CM",
        help="maximum modeled map-error addition at a zone center",
    )
    parser.add_argument(
        ...
        metavar="MS",
        help="maximum modeled sense-to-act addition at a zone center",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--rogue-vehicle-alert-radius",
        dest="rogue_vehicle_warning_radius",
        type=positive_finite_float,
        default=DEFAULT_ROGUE_VEHICLE_WARNING_RADIUS_M,
        metavar="METERS",
        help=(
            "planar ego-pedestrian radius that changes the reactive vehicle "
            "box to red and shows APPROACHING VEHICLE / STOP while boxes are "
            "enabled (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--rogue-vehicle-role-prefix",
        type=rogue_vehicle_role_prefix,
        default=DEFAULT_ROGUE_VEHICLE_ROLE_PREFIX,
        metavar="PREFIX",
        help=(
            "role-name stem identifying spawn_blocker_v5 reactive vehicles "
            "as PREFIX_<index> (default: %(default)s)"
        ),
    )
    ...
    if args.network_degradation_zones is not None:
        try:
    ...
    for option_name, value in (
        ...
        ("--spatial-map-degraded-radars", args.spatial_map_degraded_radars),
    ):
        if value > MAX_SPATIAL_MAP_ACTIVE_SENSORS:
            parser.error(
                "{} must not exceed {}".format(
                    option_name, MAX_SPATIAL_MAP_ACTIVE_SENSORS
                )
            )
    for option_name, value in (
        (
        ...
        if not math.isfinite(value) or value < 0.0:
            parser.error("{} must be finite and non-negative".format(option_name))
...
def main() -> None:
    ...
    metrics_model = LiveMetricsModel(
    ...
    spatial_sensor_subscriptions = SpatialSensorSubscriptionController()
    ...
    try:
        ...
        pygame.display.set_caption("CARLA Pedestrian Head Camera v13")
        font = pygame.font.Font(pygame.font.get_default_font(), 18)
        box_font = pygame.font.Font(pygame.font.get_default_font(), 14)
        warning_title_font = pygame.font.Font(
            pygame.font.get_default_font(), 22
        )
        warning_action_font = pygame.font.Font(
            pygame.font.get_default_font(), 27
        )
        rogue_vehicle_label_surface = box_font.render(
            ROGUE_VEHICLE_LABEL,
            True,
            BOX_LABEL_COLOR,
        )
        rogue_vehicle_warning_surface = build_rogue_vehicle_warning_surface(
            warning_title_font,
            warning_action_font,
        )
        display.fill((0, 0, 0))
        ...
        LOG.info(
            "Spawned walker id=%d and rendering RGB camera id=%d at "
            "(%.2f, %.2f, %.2f)",
            walker.id,
            camera.id,
            spawn_transform.location.x,
        ...
        LOG.info(
            "Spatial-map RGB/radar sites are virtual actor-derived glyphs; "
            "no spatial camera or radar actors were spawned"
        )
        LOG.info(
        ...
        LOG.info(
            "Ground-truth boxes/reactive-vehicle alert start %s; press B to "
            "toggle them",
            "enabled" if args.show_bboxes else "disabled",
        )
        LOG.info(
            "Rogue-vehicle AR alert: role=%s_<index> warning_radius=%.2f m; "
            "red VEHICLE box and APPROACHING VEHICLE / STOP card clear after "
            "the vehicle passes the ego pedestrian",
            args.rogue_vehicle_role_prefix,
            args.rogue_vehicle_warning_radius,
        )
        LOG.info(
            "Press U to toggle boxes/reactive-vehicle alert, ego-pedestrian "
            "route arrows, the following top-down map, and live metrics "
            "together (map radius %.1f m)",
            args.topdown_zoom_radius,
        )
        if args.disable_network_degradation:
            LOG.info(
                "Network degradation locally disabled; virtual sensor markers "
                "remain display-only while U is on"
            )
        elif args.network_degradation_zones is not None:
            LOG.info(
                "Network zones use explicit local override: %s",
                ", ".join(
                    "({:.3f},{:.3f},r={:.2f})".format(*zone)
                    for zone in args.network_degradation_zones
                ),
            )
        else:
            LOG.info(
                "Network zones follow the transactional spawn_blocker_v5 "
                "world profile dynamically; no valid profile means no zones"
            )
        LOG.info(
            "Spatial sensor policy: normal nearest complete pairs=%d; degraded "
            "nearest radars=%d; all other markers inactive. Every spatial-map "
            "site is virtual and display-only in v13; no RGB/radar actor is "
            "spawned for the map and shared-profile stream requests are ignored",
            args.spatial_map_active_sensor_pairs,
            args.spatial_map_degraded_radars,
        )
        LOG.info(
            "Network metric penalties at zone center: map latency +%.1f ms, "
            "map error +%.1f cm, sense-to-act +%.1f ms",
            args.network_degradation_map_latency_penalty_ms,
            args.network_degradation_map_accuracy_penalty_cm,
            args.network_degradation_sense_to_act_penalty_ms,
        )
        ...
        box_overlay = None
        box_counts = (0, 0)
        rogue_vehicle_warning_distance = None
        passed_rogue_vehicle_ids = set()
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key in (pygame.K_ESCAPE, pygame.K_q):
                        ...
                        spatial_sensor_subscriptions.stop_all()
                        if topdown_renderer is not None:
                            # The next render must use the post-reset/respawn
                            # ego pose. Otherwise the 5 Hz map throttle can
                            # immediately re-enable a stale pre-respawn sensor
                            # selection in this 60 Hz UI loop.
                            topdown_renderer.invalidate_dynamic_selection()
                            ...
                            if blocker is not None:
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        overlap_refresh_requested = False
                                        rogue_vehicle_warning_distance = None
                                        passed_rogue_vehicle_ids.clear()
                                        box_overlay = None
                                        box_counts = (0, 0)
                                        box_refresh_requested = boxes_enabled
                                        if route_recorder is not None:
                                    ...
                                    try:
                                        ...
                                        LOG.info(
                                            ...
                                            "(%.2f, %.2f, %.2f), yaw=%.2f; "
                                            "render camera=%d mount-z=%.2f; "
                                            "spatial RGB/radar sites remain virtual",
                                            walker.id,
                                            spawn_transform.location.x,
                                            spawn_transform.location.y,
                                            spawn_transform.location.z,
                        ...
                        LOG.info(
                            "Ground-truth bounding boxes/reactive-vehicle alert %s",
                        ...
                        if u_visuals_enabled:
                            ...
                            if not topdown_enabled:
                                (
                                    ...
                                    carla_map,
                                    args.topdown_zoom_radius,
                                    rogue_vehicle_role_prefix=(
                                        args.rogue_vehicle_role_prefix
                                    ),
                                    local_network_zones=(
                                        args.network_degradation_zones
                                    ),
                                    disable_network_degradation=(
                            ...
                            if metrics_renderer is not None:
                                metrics_renderer.close()
                            LOG.info(
                                "U visuals disabled: boxes/reactive-vehicle "
                                "alert, route arrows, top-down map, and live "
                                "metrics off"
                            )
            ...
            if topdown_enabled and topdown_renderer is not None:
                map_latency_ms = topdown_renderer.render(
                    world,
                    walker,
                    subscription_controller=spatial_sensor_subscriptions,
                )
                if map_latency_ms is not None:
                    metrics_model.note_spatial_map_latency(map_latency_ms)
                if not topdown_renderer.ready:
                    spatial_sensor_subscriptions.stop_all()
                    topdown_renderer.last_streaming_sensor_count = 0
                    topdown_enabled = False
                    LOG.warning(
                ...
                if snapshot is not None:
                    ...
                    try:
                        (
                            ...
                            rogue_vehicle_warning_distance,
                            ...
                            rogue_vehicle_label=(
                                rogue_vehicle_label_surface
                            ),
                            passed_rogue_vehicle_ids=(
                                passed_rogue_vehicle_ids
                            ),
                        )
                        box_counts = (
            ...
            if (
                boxes_enabled
                and rogue_vehicle_warning_distance is not None
            ):
                # The HUD spans the top of the RGB view, so the hazard card is
                # intentionally the final Pygame layer and remains legible.
                draw_rogue_vehicle_warning(
                    display,
                    rogue_vehicle_warning_surface,
                )
            if metrics_enabled and metrics_renderer is not None:
                degradation_strength = 0.0
                sensor_policy = "BALANCED"
                if topdown_renderer is not None:
                    try:
                        degradation_strength = network_degradation_strength(
                            walker.get_location(),
                            topdown_renderer.last_network_degradation_zones,
                        )
                    except RuntimeError:
                        degradation_strength = 0.0
                    sensor_policy = (
                        "RADAR PRIORITY"
                        if topdown_renderer.last_network_radar_priority
                        else "BALANCED"
                    )
                if not metrics_renderer.render(
                    metrics_model.snapshot(
                        network_degradation_strength_value=(
                            degradation_strength
                        ),
                        network_sensor_policy=sensor_policy,
                        network_stream_sensors=False,
                    )
                ):
                    metrics_enabled = False
