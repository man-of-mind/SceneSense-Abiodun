#!/usr/bin/env python3
"""
Coordinate-aware Physical AI CARLA scenario controller UI (v3).

V3 preserves the deterministic scenario, shared RGB sensor, camera controls,
...
* The selected pedestrian start, ordered intermediate points, and end can be
  saved independently as coordinate-authoritative navigation-surface controls.
  Its dense preview is an operator-authored interpolation for visual guidance,
  not a claim that CARLA's pedestrian navigation mesh follows each sample.
...
to this script). Pedestrian routes use ``--pedestrian-route-config`` (default:
``ego_pedestrian_route_v1.json``). Save/Load and Ctrl+S/Ctrl+O operate on the
actor family selected by the active map mode.
...
from pedestrian_route_config import (
    MAX_ROUTE_GUIDE_POINTS as PEDESTRIAN_MAX_GUIDE_POINTS,
    MAX_ROUTE_SAMPLING_RESOLUTION_M as PEDESTRIAN_MAX_SAMPLE_SPACING_M,
    MIN_ROUTE_SAMPLING_RESOLUTION_M as PEDESTRIAN_MIN_SAMPLE_SPACING_M,
    ROUTE_CONFIG_TYPE as PEDESTRIAN_ROUTE_CONFIG_TYPE,
    ROUTE_COORDINATE_SYSTEM as PEDESTRIAN_ROUTE_COORDINATE_SYSTEM,
    ROUTE_SCHEMA_VERSION as PEDESTRIAN_ROUTE_SCHEMA_VERSION,
    load_route_config as load_pedestrian_route_config,
    maps_match as pedestrian_maps_match,
    save_route_config as save_pedestrian_route_config,
)
...
LOG = logging.getLogger("physical_ai_scenario_controller_v3")
...
DEFAULT_PEDESTRIAN_ROUTE_CONFIG = SCRIPT_DIR / "ego_pedestrian_route_v1.json"
PEDESTRIAN_ROUTE_SAMPLE_SPACING_M = 1.0
PEDESTRIAN_SPAWN_HEIGHT_OFFSET_M = 0.5
PEDESTRIAN_CATALOG_WARNING_M = 1.0
PEDESTRIAN_CATALOG_REJECT_M = 5.0
PEDESTRIAN_PATH_CONTROL_TOLERANCE_M = 1.5
...
class ScenarioControllerV3(v1.ScenarioController):
    ...
    def plan_pedestrian_route_for_export(
        self,
        start: carla.Location,
        intermediate_waypoints: Sequence[carla.Location],
        end: carla.Location,
        resolution: float = PEDESTRIAN_ROUTE_SAMPLE_SPACING_M,
    ) -> Dict[str, Any]:
        """Build a dense visual guide through pedestrian controls.

        CARLA exposes random navigation-surface samples and accepts successive
        walker destinations, but it does not expose a pedestrian equivalent of
        ``GlobalRoutePlanner.trace_route``.  The returned ``planned_path`` is
        therefore an operator-authored, piecewise-linear interpolation used by
        the UI and head-camera guide.  The selected start/via/end controls stay
        authoritative and the runtime walker controller continues to navigate
        between those controls on CARLA's navigation mesh.
        """
        sampling_resolution = float(resolution)
        if not (
            math.isfinite(sampling_resolution)
            and PEDESTRIAN_MIN_SAMPLE_SPACING_M
            <= sampling_resolution
            <= PEDESTRIAN_MAX_SAMPLE_SPACING_M
        ):
            raise ValueError(
                "pedestrian route sampling resolution must be between {:.2f} "
                "and {:.1f} meters".format(
                    PEDESTRIAN_MIN_SAMPLE_SPACING_M,
                    PEDESTRIAN_MAX_SAMPLE_SPACING_M,
                )
            )
        controls = self._dedupe_route_locations(
            [v1.copy_location(start)]
            + [v1.copy_location(location) for location in intermediate_waypoints]
            + [v1.copy_location(end)],
            minimum_distance_m=0.25,
        )
        if len(controls) < 2:
            raise ValueError("pedestrian route start and end must be distinct")

        planned_path: List[carla.Location] = [v1.copy_location(controls[0])]
        for segment_start, segment_end in zip(controls, controls[1:]):
            distance = float(segment_start.distance(segment_end))
            step_count = distance / sampling_resolution
            ...
            if len(planned_path) + steps > PEDESTRIAN_MAX_GUIDE_POINTS:
                raise ValueError(
                    "pedestrian route would exceed the {}-point visual guide "
                    "limit; choose a larger sampling resolution or closer "
                    "controls".format(PEDESTRIAN_MAX_GUIDE_POINTS)
                )
            for step in range(1, steps + 1):
                ...
                planned_path.append(
                    carla.Location(
                        x=segment_start.x
                        + (segment_end.x - segment_start.x) * fraction,
                        y=segment_start.y
                        + (segment_end.y - segment_start.y) * fraction,
                        z=segment_start.z
                        + (segment_end.z - segment_start.z) * fraction,
                    )
                )

        def heading(first: carla.Location, second: carla.Location) -> float:
            return math.degrees(
                math.atan2(float(second.y - first.y), float(second.x - first.x))
            )

        return {
            "start_transform": carla.Transform(
                v1.copy_location(controls[0]),
                carla.Rotation(yaw=heading(controls[0], controls[1])),
            ),
            "intermediate_waypoints": [
                v1.copy_location(location) for location in controls[1:-1]
            ],
            "end_transform": carla.Transform(
                v1.copy_location(controls[-1]),
                carla.Rotation(yaw=heading(controls[-2], controls[-1])),
            ),
    ...
    def _activate_scripted_ego_vehicle(
        ...
        """Reserve v3 ego authority for BasicAgent; TM remains NPC-only."""
        del route_path
    ...
    def cleanup(self, remove_tick_callback: bool = False) -> None:
        """Route every v3 reset, failure, replay, and shutdown through stop_demo."""
...
class TopDownMapSelector:
    ...
    def draw(
        ...
        pedestrian_route_preview: Sequence[carla.Location] = (),
    ) -> None:
        ...
        if len(pedestrian_route_preview) >= 2:
            preview_points = [
                self.world_to_screen(location)
                for location in pedestrian_route_preview
            ]
            pygame.draw.lines(screen, v1.COLOR_BG, False, preview_points, 6)
            pygame.draw.lines(
                screen, COLOR_ROUTE_PEDESTRIAN, False, preview_points, 3
            )
        elif pedestrian_start is not None and pedestrian_end is not None:
            pygame.draw.lines(
                screen,
...
class ScenarioUIV3(v1.ScenarioUI):
    """V3 layout with independently persistent vehicle and pedestrian routes."""

    def __init__(
        self,
        controller: ScenarioControllerV3,
        base_config: Dict[str, Any],
        route_config_path: Path,
        pedestrian_route_config_path: Path,
    ) -> None:
        self.route_config_path = Path(route_config_path).resolve()
        self.pedestrian_route_config_path = Path(
            pedestrian_route_config_path
        ).resolve()
        self.route_status = "Vehicle route file: {}".format(
            self.route_config_path
        )
        self.vehicle_route_preview: List[carla.Location] = []
        self.pedestrian_route_preview: List[carla.Location] = []
        self.loaded_route_config: Optional[Dict[str, Any]] = None
        self.loaded_pedestrian_route_config: Optional[Dict[str, Any]] = None
        ...
        if self.pedestrian_route_config_path.is_file():
            self._guard(self._load_pedestrian_route)
        pygame.display.set_caption(
            "CARLA Physical AI Scenario Controller v3"
        )
    ...
    def _build_controls(self) -> None:
        ...
        self.load_route_button = v1.Button(
            ...
            lambda: self._guard(self._load_active_route),
        )
        self.save_route_button = v1.Button(
            ...
            lambda: self._guard(self._save_active_route),
        )
    ...
    @property
    def typed_controller(self) -> ScenarioControllerV3:
    ...
    def _nearest_catalog_index(
        ...
        if horizontal_only:
            distances = [
                math.hypot(
                    float(point_location(point).x - location.x),
                    float(point_location(point).y - location.y),
                )
                for point in points
            ]
        else:
    ...
    def _invalidate_pedestrian_route_preview(self) -> None:
        self.pedestrian_route_preview = []
        self.loaded_pedestrian_route_config = None
        self.route_status = (
            "Pedestrian route changed; Save route to rebuild preview"
        )

    def _active_route_family(self) -> str:
        return "pedestrian" if self.map_mode.startswith("pedestrian_") else "vehicle"

    def _save_active_route(self) -> None:
        if self._active_route_family() == "pedestrian":
            self._save_pedestrian_route()
        else:
            self._save_vehicle_route()

    def _load_active_route(self) -> None:
        if self._active_route_family() == "pedestrian":
            self._load_pedestrian_route()
        else:
            self._load_vehicle_route()
    ...
    @staticmethod
    def _pedestrian_nav_location_from_start(
        start_transform: carla.Transform,
        spawn_height_offset_m: float,
    ) -> carla.Location:
        location = v1.copy_location(start_transform.location)
        location.z -= float(spawn_height_offset_m)
        return location

    @staticmethod
    def _path_visits_controls_in_order(
        path: Sequence[carla.Location],
        controls: Sequence[carla.Location],
        tolerance_m: float,
    ) -> bool:
        if len(path) < 2 or len(controls) < 2:
            return False
        if (
            path[0].distance(controls[0]) > tolerance_m
            or path[-1].distance(controls[-1]) > tolerance_m
        ):
            return False
        if len(controls) == 2:
            return True
        control_index = 1
        for location in path[1:-1]:
            if location.distance(controls[control_index]) <= tolerance_m:
                control_index += 1
                if control_index == len(controls) - 1:
                    return True
        return False

    def _save_pedestrian_route(self) -> None:
        """Atomically save coordinate-authoritative pedestrian route controls."""
        start_index = int(self.ped_start.get())
        end_index = int(self.ped_end.get())
        if not 0 <= start_index < len(self.pedestrian_points):
            raise ValueError("pedestrian start selection is unavailable")
        if not 0 <= end_index < len(self.pedestrian_points):
            raise ValueError("pedestrian end selection is unavailable")
        invalid_via = [
            index
            for index in self.pedestrian_waypoint_indices
            if not 0 <= int(index) < len(self.pedestrian_points)
        ]
        if invalid_via:
            raise ValueError(
                "pedestrian route contains stale waypoint indices: {}".format(
                    invalid_via
                )
            )

        start_nav = v1.copy_location(self.pedestrian_points[start_index])
        end_nav = v1.copy_location(self.pedestrian_points[end_index])
        selected_vias = [
            v1.copy_location(self.pedestrian_points[int(index)])
            for index in self.pedestrian_waypoint_indices
        ]
        resolution = PEDESTRIAN_ROUTE_SAMPLE_SPACING_M
        planned = self.typed_controller.plan_pedestrian_route_for_export(
            start_nav,
            selected_vias,
            end_nav,
            resolution,
        )
        # The start transform is the exact actor pose consumed by the manual
        # pedestrian client. Ground guidance and every later control remain on
        # the navigation surface, avoiding floating route paint.
        start_transform = v1.copy_transform(planned["start_transform"])
        start_transform.location.z += PEDESTRIAN_SPAWN_HEIGHT_OFFSET_M
        planned_via_indices = [
            self._nearest_catalog_index(
                location, self.pedestrian_points, horizontal_only=True
            )[0]
            for location in planned["intermediate_waypoints"]
        ]
        map_name = str(self.typed_controller.map.name)
        route_data = {
            ...
            "name": "{} ego pedestrian route".format(Path(map_name).name),
            "map": map_name,
            "coordinate_system": PEDESTRIAN_ROUTE_COORDINATE_SYSTEM,
            "route_sampling_resolution_m": resolution,
            "spawn_height_offset_m": PEDESTRIAN_SPAWN_HEIGHT_OFFSET_M,
            "start": self._transform_payload(start_transform),
            "intermediate_waypoints": [
                self._location_payload(location)
                for location in planned["intermediate_waypoints"]
            ],
            ...
            "ui_selection": {
                ...
                "navigation_seed": int(self.seed.get()),
                "navigation_sample_count": 128,
                "pedestrian_start_navigation_index": start_index,
                "pedestrian_end_navigation_index": end_index,
                "pedestrian_waypoint_indices": [
                    int(index) for index in planned_via_indices
                ],
                "navigation_catalog_size": len(self.pedestrian_points),
            },
            "created_utc": datetime.now(timezone.utc).isoformat(),
        }
        normalized = save_pedestrian_route_config(
            self.pedestrian_route_config_path, route_data
        )
        self.loaded_pedestrian_route_config = normalized
        self.pedestrian_route_preview = [
            v1.copy_location(location) for location in planned["planned_path"]
        ]
        self.route_status = "Saved {} pedestrian guide points to {}".format(
            len(self.pedestrian_route_preview),
            self.pedestrian_route_config_path,
        )
        self.controller.status = self.route_status

    def _load_pedestrian_route(self) -> None:
        """Load pedestrian route coordinates and map them to this nav catalog."""
        route = load_pedestrian_route_config(self.pedestrian_route_config_path)
        current_map = str(self.typed_controller.map.name)
        if not pedestrian_maps_match(route["map"], current_map):
            raise ValueError(
                "pedestrian route map {!r} does not match loaded CARLA map "
                "{!r}".format(route["map"], current_map)
            )
        hints = route.get("ui_selection", {})
        if not isinstance(hints, dict):
            hints = {}
        saved_seed = hints.get("navigation_seed")
        if not isinstance(saved_seed, bool):
            try:
                saved_seed = int(saved_seed)
            except (TypeError, ValueError, OverflowError):
                saved_seed = None
        if saved_seed is not None and saved_seed != int(self.seed.get()):
            self.seed.value = v1.clamp(
                saved_seed, self.seed.minimum, self.seed.maximum
            )
            self._refresh_pedestrian_preview(preserve_selection=False)
        if not self.pedestrian_points:
            raise ValueError("loaded CARLA world has no pedestrian route catalog")

        spawn_height_offset = float(route.get("spawn_height_offset_m", 0.0))
        start_transform = self._payload_transform(route["start"])
        start_nav = self._pedestrian_nav_location_from_start(
            start_transform, spawn_height_offset
        )
        end_transform = self._payload_transform(route["end"])
        start_index, start_error = self._nearest_catalog_index(
            start_nav,
            self.pedestrian_points,
            hints.get("pedestrian_start_navigation_index"),
            horizontal_only=True,
        )
        end_index, end_error = self._nearest_catalog_index(
            end_transform.location,
            self.pedestrian_points,
            hints.get("pedestrian_end_navigation_index"),
            horizontal_only=True,
        )
        via_hints = hints.get("pedestrian_waypoint_indices", [])
        if not isinstance(via_hints, list):
            via_hints = []
        waypoint_indices: List[int] = []
        mapping_errors = [start_error, end_error]
        saved_vias = [
            self._payload_location(payload)
            for payload in route["intermediate_waypoints"]
        ]
        for order, waypoint in enumerate(saved_vias):
            index_hint = via_hints[order] if order < len(via_hints) else None
            index, mapping_error = self._nearest_catalog_index(
                waypoint,
                self.pedestrian_points,
        ...
        largest_mapping_error = max(mapping_errors) if mapping_errors else 0.0
        if largest_mapping_error > PEDESTRIAN_CATALOG_REJECT_M:
            raise ValueError(
                "saved pedestrian controls are up to {:.1f} m from this "
                "navigation catalog; refusing ambiguous mapping".format(
                    largest_mapping_error
                )
            )

        self.ped_start.value = float(start_index)
        self.ped_end.value = float(end_index)
        self.pedestrian_waypoint_indices = waypoint_indices
        controls = [start_nav] + saved_vias + [end_transform.location]
        preview = [
            self._payload_location(location)
            for location in route.get("planned_path", [])
        ]
        if not self._path_visits_controls_in_order(
            preview, controls, PEDESTRIAN_PATH_CONTROL_TOLERANCE_M
        ):
            LOG.warning(
                "Stored pedestrian planned_path is stale; rebuilding its "
                "operator-authored interpolation"
            )
            planned = self.typed_controller.plan_pedestrian_route_for_export(
                start_nav,
                saved_vias,
        ...
        self.loaded_pedestrian_route_config = route
        self.pedestrian_route_preview = [
            v1.copy_location(location) for location in preview
        ]
        if largest_mapping_error > PEDESTRIAN_CATALOG_WARNING_M:
            LOG.warning(
                "Loaded pedestrian route catalog mapping is as far as %.1f m "
                "from a saved control coordinate",
                largest_mapping_error,
            )
        self.route_status = "Loaded {} pedestrian guide points from {}".format(
            len(self.pedestrian_route_preview),
            self.pedestrian_route_config_path,
        )
        self.controller.status = self.route_status
    ...
    def _update_map_mode_labels(self) -> None:
        ...
        route_family = self._active_route_family()
        if hasattr(self, "load_route_button"):
            self.load_route_button.label = (
                "Load ped" if route_family == "pedestrian" else "Load veh"
            )
            self.save_route_button.label = (
                "Save ped" if route_family == "pedestrian" else "Save veh"
            )
    ...
    def _undo_active_waypoint(self) -> None:
        ...
        if active_waypoints:
            ...
            elif self.map_mode == "pedestrian_waypoints":
                self._invalidate_pedestrian_route_preview()
        self._update_map_mode_labels()
    ...
    def _refresh_pedestrian_preview(
        self, preserve_selection: bool = False
    ) -> None:
        """Rebuild the exact seeded nav catalog used by the next scenario.

        A seed change invalidates all index identities. Coordinate restoration
        is intentionally not attempted here: only a saved pedestrian route has
        durable coordinates, and loading that route performs verified mapping.
        """
        previous_start = int(self.ped_start.get()) if preserve_selection else 0
        previous_end = int(self.ped_end.get()) if preserve_selection else 1
        self.pedestrian_points = self.typed_controller.navigation_preview(
            int(self.seed.get())
        )
        if len(self.pedestrian_points) < 2:
            raise RuntimeError(
                "CARLA navigation mesh returned fewer than two pedestrian points"
            )
        self.map_selector.set_pedestrian_points(self.pedestrian_points)
        maximum = len(self.pedestrian_points) - 1
        self.ped_start.maximum = maximum
        self.ped_end.maximum = maximum
        self.ped_start.value = v1.clamp(previous_start, 0, maximum)
        self.ped_end.value = v1.clamp(previous_end, 0, maximum)
        if int(self.ped_end.get()) == int(self.ped_start.get()):
            self.ped_end.value = float((int(self.ped_start.get()) + 1) % len(
                self.pedestrian_points
            ))
    ...
    def _apply_map_selection(self, index: int) -> None:
        ...
        if target.value != selected_index:
            ...
            if self.map_mode in ("vehicle_start", "vehicle_end"):
                self._invalidate_vehicle_route_preview()
            elif self.map_mode in ("pedestrian_start", "pedestrian_end"):
                self._invalidate_pedestrian_route_preview()
    ...
    def _handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_s and mods & pygame.KMOD_CTRL:
                self._guard(self._save_active_route)
            elif event.key == pygame.K_o and mods & pygame.KMOD_CTRL:
                self._guard(self._load_active_route)
        ...
        for control in self.config_controls:
            ...
            if control in (self.vehicle_start, self.vehicle_end):
                self._invalidate_vehicle_route_preview()
            elif control in (self.ped_start, self.ped_end):
                self._invalidate_pedestrian_route_preview()
    ...
    def _draw_panel(self) -> None:
        ...
        self.screen.blit(
            self.small_font.render(
                "Coordinate-aware deterministic controller v3",
                True,
                v1.COLOR_MUTED,
            ),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_PEDESTRIAN_ROUTE_CONFIG,
        help=(
            "ego-pedestrian route JSON used by context-sensitive Load/Save "
            "and loaded automatically when it exists (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--log-level",
...
def main() -> int:
    ...
    try:
        ...
        ui = ScenarioUIV3(
            ...
            args.pedestrian_route_config.resolve(),
        )
        ui.run()
