...
def mean_or_blank(values: Sequence[float]) -> float | str:
    ...
    return float(sum(finite) / len(finite)) if finite else ""


class PassiveSplitCollector:
    """Two sensors and a bounded front worker attached to Route B's exact ego."""
    ...
    def _spawn_sensors(self) -> None:
        ...
        self.sensors = [self.camera, self.radar]
    ...
    def _map_snapshot(self, expected_frame: int) -> list[dict[str, Any]] | None:
        url = f"http://127.0.0.1:{self.map_api_port}/api/spatial_map/latest"
        try:
            with urllib.request.urlopen(url, timeout=0.5) as response:
                value = json.loads(response.read().decode("utf-8"))
        except (OSError, urllib.error.URLError, json.JSONDecodeError):
            return None
        if int(value.get("frame_id") or -1) != int(expected_frame):
            return None
        predictions: list[dict[str, Any]] = []
        for obj in value.get("raw_spatial_map_objects", []):
    ...
    def finish(self) -> bool:
        ...
        try:
            ...
            sensor_ok = False
        self.cleanup_ok = sensor_ok and not self.worker.is_alive() and not self.feedback_worker.is_alive()
    ...
    def write_perception(self) -> None:
        ...
        with path.open("x", newline="", encoding="utf-8") as handle:
            ...
            writer.writeheader()
            with self.gt_lock:
                frames = sorted(self.source_gt)
                for frame_id in frames:
                    predictions = self.installed_predictions.get(frame_id)
                    for class_name in ("vehicle", "person"):
                        gt = [row for row in self.source_gt[frame_id] if row.get("class_name") == class_name]
                        preds = [row for row in (predictions or []) if row.get("class_name") == class_name]
                        matches = greedy_match_predictions(preds, gt, max_distance_m=5.0, class_aware=True) if predictions is not None else []
                        tp = len(matches)
                        fp = len(preds) - tp if predictions is not None else 0
                        fn = (len(gt) - tp) if predictions is not None else 0
                        ...
                        aligned_matches = (
                            greedy_match_predictions(
                                preds, aligned_truth, max_distance_m=5.0, class_aware=True
                            )
                            if predictions is not None and aligned_truth
                            else []
                        )
                        aligned_errors = [
                        ...
                        writer.writerow(
                            {
                                ...
                                "precision": (tp / denom_pred) if denom_pred else "",
                                "recall": (tp / denom_gt) if denom_gt else "",
                                "valid_empty": int(predictions is not None and not preds and not gt),
                                "coverage": (tp / denom_gt) if denom_gt else int(predictions is not None),
                                "source_time_world_xy_error_m": mean_or_blank(source_errors),
                                "aligned_world_xy_error_m": mean_or_blank(aligned_errors),
                                "dimension_error_m": mean_or_blank(dimension_errors),
                                "footprint_iou": "", "segmentation_iou": "",
                            }
                        )
