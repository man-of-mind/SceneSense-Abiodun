#!/usr/bin/env python3
"""Dedicated live engine for the bounded UE-N3D -3.5 dB refinement.
...
cold starts using run-local configs.  The -3.5 dB candidate is baked before UE
...
The 7.5 +/- 0.5 dB achieved-PUSCH band is consistency-only provenance from a
...
from rl_agent import ue_n3d_oai_ul_cold_attach_refinement_v1 as plan  # noqa: E402


DEFAULT_CONFIG = (
    ROOT / "rl_agent/configs/ue_n3d_oai_ul_cold_attach_refinement_live_v1.json"
)
CANONICAL_PLAN_CONFIG = (
    ROOT / "rl_agent/configs/ue_n3d_oai_ul_cold_attach_refinement_v1.json"
)
...
SCHEMA = "scenesense.ue_n3d_oai_ul_cold_attach_refinement_live_config.v1"
PLAN_BLOCKED = "UE_N3D_LIVE_PLAN_FROZEN_PREREQUISITES_PENDING"
PLAN_READY = "UE_N3D_LIVE_PLAN_FROZEN_READY_FOR_EXPLICIT_EXECUTE_LIVE"
REP_PASSED = "UE_N3D_COLD_ATTACH_SERVICE_REPETITION_PASSED"
REP_ATTACH_FAILED = (
    "UE_N3D_COLD_ATTACH_REPETITION_VALID_OPERATIONAL_SCREEN_NONCONFIRMATION"
)
REP_SERVICE_FAILED = "UE_N3D_COLD_ATTACH_REPETITION_VALID_SERVICE_FAILURE"
REP_SNR_MISMATCH = "UE_N3D_COLD_ATTACH_REPETITION_VALID_SNR_CONSISTENCY_MISMATCH"
REP_UNCONFIRMED = "UE_N3D_COLD_ATTACH_REPETITION_EVIDENCE_UNCONFIRMED"
CAMPAIGN_PASSED = "UE_N3D_COLD_ATTACH_REFINEMENT_3_OF_3_PASSED_REVIEW_REQUIRED"
CAMPAIGN_NOT_3_OF_3 = "UE_N3D_COLD_ATTACH_REFINEMENT_NOT_3_OF_3_REVIEW_REQUIRED"
RESTORE_FAILED = "UE_N3D_FAILED_RESTORE"
FAILED = "FAILED"
...
EXPECTED_N3B_ADJUDICATION = {
    ...
    "terminal": (
        "UE_N3B_OUTCOME_ADJUDICATED_N3D_ELIGIBLE_REVIEW_REQUIRED.json"
    ),
    "terminal_sha256": (
    ...
    "required_status": (
        "UE_N3B_OUTCOME_ADJUDICATED_N3D_ELIGIBLE_REVIEW_REQUIRED"
    ),
    "n3d_eligibility_status": (
        "UE_N3B_VALID_COLD_ATTACH_FAILURE_ACCEPTED_FOR_N3D"
    ),
    "n3d_selected_command_db": -3.5,
}
...
def validate_output_contract(config: Mapping[str, Any]) -> None:
    ...
    require(
        ...
        "N3D live output leaf contract drift",
    )
...
def validate_config(
    ...
    require(config.get("schema") == SCHEMA, "unexpected N3D live config schema")
    require(
        config.get("experiment_id")
        == "ue_n3d_oai_ul_cold_attach_refinement_live_v1",
        "N3D live experiment identity drift",
    )
    require(
        config["paths"]["output_root"]
        == "rl_agent/experiments/ue_n3d_oai_ul_cold_attach_refinement_live_v1",
        "N3D live output root drift",
    )
    validate_output_contract(config)
    require(n2.sha256(CANONICAL_PLAN_CONFIG) == CANONICAL_PLAN_CONFIG_SHA256,
            "sealed N3D PREPARE config drift")
    canonical = load_json(CANONICAL_PLAN_CONFIG)
    require(_scientific_projection(config) == _scientific_projection(canonical),
            "N3D live scientific contract differs from sealed PREPARE contract")
    require(
    ...
    normalized["experiment_id"] = "ue_n3d_oai_ul_cold_attach_refinement_v1"
    plan.validate_config(
        normalized, verify_hashes=verify_hashes, require_live=require_live,
    )
    required_live_path = "rl_agent/ue_n3d_oai_ul_cold_attach_refinement_live_v1.py"
    matches = [
        row for row in config["runtime_seals"] if row.get("path") == required_live_path
    ]
    require(len(matches) == 1, "N3D live runner seal is absent or duplicated")
    if verify_hashes:
        require(
            n2.sha256(plan.resolve_repo_path(required_live_path))
            == matches[0]["sha256"],
            "N3D live runner seal drift",
        )
...
def aggregate_results(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    require(len(results) == 3, "N3D requires three completed repetition results")
    require(
        all(row.get("evidence_valid_for_aggregation") is True for row in results),
        "N3D contains invalid repetition evidence",
    )
...
class ColdAttachLiveRepetition(n3b.ColdAttachRepetitionRunner):
    """One fresh-RAN -3.5 dB cold-attach/service repetition."""
    ...
    def materialize_configs(self) -> tuple[Path, Path]:
        ...
        runtime.mkdir()
        channel_path = runtime / "effective_channel_cold_attach_minus3p5.conf"
        gnb_path = runtime / "effective_gnb_cold_attach_minus3p5.conf"
        ue_path = runtime / "effective_ue_cold_attach_minus3p5.conf"
    ...
    def write_manifest_terminal(self, status: str, summary: Mapping[str, Any]) -> None:
        ...
        for path in sorted(self.output_dir.rglob("*")):
            if (
                ...
                or path.name.startswith("UE_N3D_")
            ):
                continue
            files.append({
        ...
        n2.atomic_json(manifest_path, {
            "schema": "scenesense.ue_n3d_cold_attach_repetition_manifest.v1",
            "status": final_status,
            "repetition_index": self.repetition_index,
            "commanded_noise_power_db": plan.CANDIDATE_COMMAND_DB,
...
def verify_repetition_evidence(
    ...
    for payload, label in (
        ...
        require(
            math.isclose(
                float(payload.get("commanded_noise_power_db", math.nan)), -3.5,
            ),
    ...
    return {
        "status": "VERIFIED_N3D_REPETITION_EVIDENCE",
...
class CampaignRunner:
    ...
    def manifest_terminal(self, status: str, summary: Mapping[str, Any]) -> None:
        ...
        manifest_path = self.output_dir / EXPECTED_OUTPUT["manifest"]
        n2.atomic_json(manifest_path, {
            "schema": "scenesense.ue_n3d_cold_attach_live_campaign_manifest.v1",
            "status": status,
            "config_sha256": n2.sha256(self.config_path),
            "runner_sha256": n2.sha256(Path(__file__).resolve()),
            "engine_runner_sha256": n2.sha256(Path(calibration.__file__).resolve()),
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            for row in rows:
                ...
                directory = (
                    self.output_dir / "repetitions" / f"rep_{index:02d}_minus3p5_cold"
                )
            ...
            summary = {
                ...
                **aggregation,
                ...
                "n3b_adjudication_predecessor": review,
                "command_ladder_expectation_predecessor": expectation,
                "next": "REVIEW_N3D_EVIDENCE_NO_AUTOMATIC_BOUND_PROMOTION",
            }
            self.manifest_terminal(status, summary)
            return 0
        except (Exception, KeyboardInterrupt) as exc:
