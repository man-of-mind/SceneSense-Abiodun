#!/usr/bin/env python3
"""Replay four frozen profiles in four fresh qualified OAI/RFsim lifecycles.
...
SUCCESS_TERMINAL = "SPLITFUSION_PHASE14B_FOUR_PROFILE_REPLAY_COMPLETE"
...
class FourProfileReplay(phase14a.AttachedCalibration):
    ...
    def __init__(
        ...
        *,
        ...
        require(float(self.base_radio_config["traffic"]["fps"]) == float(traffic["fps"]), "traffic FPS differs from Phase-14A")
        require(int(self.base_radio_config["traffic"]["frame_bytes"]) == int(traffic["frame_bytes"]), "traffic payload differs from Phase-14A")
        require(int(self.base_radio_config["traffic"]["chunk_bytes"]) == int(traffic["chunk_bytes"]), "traffic chunk size differs from Phase-14A")
        require(
