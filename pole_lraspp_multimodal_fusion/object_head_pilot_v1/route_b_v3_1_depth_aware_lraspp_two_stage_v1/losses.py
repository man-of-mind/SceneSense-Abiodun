...
def representation_losses(model: torch.nn.Module, batch: Mapping[str, Any],
                          weights: Mapping[str, float]) -> tuple[torch.Tensor, dict[str, torch.Tensor], dict[str, int], dict[str, Any]]:
    """Stage-1 objective. No private object module is executed."""
    inputs = batch["input"].to(next(model.parameters()).device, non_blocking=True)
    labels = batch["segmentation"].to(inputs.device, non_blocking=True)
    outputs = model.representation_outputs(inputs)
    segmentation, segmentation_parts, segmentation_denoms = segmentation_losses(outputs["out"], labels)
    depth_parts, depth_denoms = dense_losses(
        outputs["dense_depth_log1p"], batch["dense_depth"], batch["dense_valid"], batch["radar_points"],
    )
    parts: dict[str, torch.Tensor] = {"segmentation": segmentation}
    parts.update(segmentation_parts); parts.update(depth_parts)
    total = (float(weights["segmentation"]) * parts["segmentation"]
             + float(weights["dense_depth"]) * parts["dense_depth"]
             + float(weights["radar_consistency"]) * parts["radar_consistency"])
    return total, parts, {**segmentation_denoms, **depth_denoms}, outputs


def private_object_losses(model: torch.nn.Module, batch: Mapping[str, Any],
                          weights: Mapping[str, float]) -> tuple[torch.Tensor, dict[str, torch.Tensor], dict[str, int], dict[str, Any]]:
    """Stage-2 objective. The frozen representation is evaluated under no_grad."""
    inputs = batch["input"].to(next(model.parameters()).device, non_blocking=True)
    outputs = model.object_outputs(inputs)
    parts, denominators = actor_object_losses(
        outputs, batch["heatmap"].to(inputs.device, non_blocking=True), batch["owners"],
        model.depth_anchors, model.depth_delta,
    )
    names = ("heatmap", "subcell", "box_center_delta", "box_wh", "physical_ray",
             "depth_bin", "depth_residual", "endpoint", "dimensions", "yaw", "parked",
             "radar_support")
    total = sum(float(weights[name]) * parts[name] for name in names)
    return total, parts, denominators, {"objects": outputs}
