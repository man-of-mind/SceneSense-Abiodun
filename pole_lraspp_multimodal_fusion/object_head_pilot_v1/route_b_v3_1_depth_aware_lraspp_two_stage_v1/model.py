...
class DepthAwareLRASPP(torch.nn.Module):
    ...
    def representation(self, value: torch.Tensor) -> torch.Tensor:
        """The only path from deployable inputs to the shared task representation."""
        return self.depth_neck(self.encode_front(value))

    def representation_outputs(self, value: torch.Tensor) -> dict[str, torch.Tensor]:
        """Stage-1 path: deliberately does not execute either private object branch."""
        shared = self.representation(value)
        segmentation = F.interpolate(
            self.segmentation(shared), size=(MODEL_SIZE_WH[1], MODEL_SIZE_WH[0]),
            mode="bilinear", align_corners=False,
        )
        return {"out": segmentation, "dense_depth_log1p": self.dense_depth(shared)}

    def object_outputs(self, value: torch.Tensor) -> OrderedDict[str, OrderedDict[str, torch.Tensor]]:
        """Stage-2 path: representation is frozen and is never placed on an autograd tape."""
        with torch.no_grad():
            shared = self.representation(value)
        return OrderedDict((("vehicle", self.vehicle(shared)), ("person", self.person(shared))))
...
def configure_two_stage(model: DepthAwareLRASPP, stage: str) -> None:
    if stage not in {"stage1", "stage2"}:
        raise ValueError(stage)
    for name, parameter in model.named_parameters():
        object_private = name.startswith("vehicle.") or name.startswith("person.")
        parameter.requires_grad = (not object_private) if stage == "stage1" else object_private
    model.train()
    freeze_bn_running_state(model)


def reset_private_object_branches(model: DepthAwareLRASPP, seed: int) -> None:
    """Replace both private branches under a deterministic, registered RNG seed."""
    devices = list({parameter.device for parameter in model.parameters()})
    with torch.random.fork_rng(devices=[device.index for device in devices if device.type == "cuda"]):
        torch.manual_seed(int(seed))
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(int(seed))
        model.vehicle = ObjectBranch("vehicle").to(device=model.depth_anchors.device)
        model.person = ObjectBranch("person").to(device=model.depth_anchors.device)
