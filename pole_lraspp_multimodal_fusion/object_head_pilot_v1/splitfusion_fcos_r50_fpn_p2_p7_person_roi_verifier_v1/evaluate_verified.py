...
def main() -> int:
    parser = argparse.ArgumentParser(description="Score one completed person-verified prediction directory")
    ...
    if (not complete.is_file()
            or complete.read_text(encoding="utf-8") != "PERSON_ROI_VERIFIER_INFERENCE_COMPLETE\n"):
        raise RuntimeError("person-verifier prediction directory is incomplete")
    inference = json.loads((prediction / "inference_manifest.json").read_text(encoding="utf-8"))
    if (inference.get("schema") != "splitfusion_fcos_person_roi_verifier_inference_v1"
            or float(inference.get("derived_threshold", -1.0)) != 0.20
            or inference.get("checkpoint_sha256") != FROZEN_CHECKPOINT_SHA256
            or inference.get("candidate_set") != "unchanged_post_original_nms"
            or inference.get("vehicle_fields_and_scores_changed") is not False):
        raise RuntimeError("person-verifier inference contract drift")
