...
`cache_join.py` streams paired shards and fails closed on the locked hashes, corpus counts, checkpoint provenance, episode partition, frame identities, candidate counts/order, and FP32 base scores. `train_selector.py` implements the fixed five-epoch, deterministic 1:3 loss-sampling procedure while keeping all candidates in attention context. Its one holdout threshold must achieve precision and recall of at least 0.80 both jointly and in each untouched holdout episode before a checkpoint permits validation.
