...
def load_frozen_runtime(device: torch.device) -> FrozenRuntime:
    """Load the exact recovered epoch-26 model and freeze every base parameter."""
    ...
    tau = float(checkpoint["recovery"]["selected_tau"])
    base = load_base()
    priors = base.common.load_json(BASE_EXPERIMENT / "TRAIN_ONLY_PRIORS.json")
    model, _report = build_recovery_model(priors, tau, device)
    ...
    base_config = base.common.load_json(base.common.CONFIG_PATH)
    dataset_root = (ROOT / base_config["dataset_root"]).resolve(strict=True)
