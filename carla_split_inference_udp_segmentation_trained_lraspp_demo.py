...
"""
CARLA split-inference UDP demo for a fine-tuned traffic-light-pole LR-ASPP model.

This is an additive copy of carla_split_inference_udp_segmentation_demo.py
prepared for checkpoints produced by pole_lraspp_training. It keeps the
localhost UDP split-inference and feature-compression path, but defaults to a
3-class CARLA-trained LR-ASPP MobileNetV3 head:

    0 = background, 1 = vehicle, 2 = person

Pass either --seg-weights-path /path/to/best.pt or --trained-experiment-dir
/path/to/experiments/pole_lraspp_training/<timestamp>_pole_lraspp_training.
The script can use CARLA semantic-segmentation GT for per-frame mIoU and
transport/latency metrics while rendering high-contrast overlays.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(seg_pretrained=False, collect_metrics=True, live_plot=False)
    parser.add_argument(
        "--seg-weights-path",
        default="",
        help="Optional state_dict checkpoint for the segmentation model. For pole_lraspp_training, pass checkpoints/<trial>/best.pt.",
    )
    parser.add_argument(
    ...
    semantic_gt_group.add_argument(
        ...
        dest="enable_semantic_gt",
        action="store_false",
        help="Skip the GT-camera + mIoU instrumentation.",
    )
    parser.set_defaults(enable_semantic_gt=True, per_level_compress_probe=True)
    parser.add_argument(
...
def _load_checkpoint_blob(path: str) -> Dict[str, object]:
    if not path:
        return {}
    loaded = torch.load(Path(path).expanduser(), map_location="cpu")
    return loaded if isinstance(loaded, dict) else {"state_dict": loaded}


def _checkpoint_state_dict(checkpoint: Dict[str, object]) -> Dict[str, torch.Tensor]:
    ...
    if not isinstance(state, dict):
        raise ValueError("Segmentation checkpoint did not contain a state_dict.")
    return {str(k).removeprefix("module."): v for k, v in state.items()}


def _find_best_checkpoint_in_experiment(exp_dir: Path) -> Path:
        ...
        try:
            ...
            if best:
                ...
                if not best_path.is_absolute():
                    best_path = (exp_dir / best_path).resolve()
                if best_path.exists():
                    return best_path
...
def apply_trained_checkpoint_args(args: argparse.Namespace) -> None:
    if not str(args.seg_weights_path or "").strip() and str(args.trained_experiment_dir or "").strip():
        args.seg_weights_path = str(_find_best_checkpoint_in_experiment(Path(args.trained_experiment_dir).expanduser()))

    if not str(args.seg_weights_path or "").strip():
        print(
            "Warning: no trained checkpoint was provided. The model will run with random 3-class weights.",
            file=sys.stderr,
        )
        return

    checkpoint = _load_checkpoint_blob(str(args.seg_weights_path))
    trial = checkpoint.get("trial") if isinstance(checkpoint.get("trial"), dict) else {}
    if isinstance(trial, dict):
        args.seg_num_classes = int(checkpoint.get("num_classes", args.seg_num_classes))
        if args.use_checkpoint_input_size and "input_size" in trial and "input_size" not in checkpoint:
            checkpoint["input_size"] = trial["input_size"]

    if args.use_checkpoint_input_size and isinstance(checkpoint.get("input_size"), (list, tuple)):
        input_size = list(checkpoint["input_size"])  # type: ignore[index]
        if len(input_size) >= 2:
            args.seg_input_width = int(input_size[0])
            args.seg_input_height = int(input_size[1])


def segmentation_label_names(args: argparse.Namespace) -> Tuple[str, ...]:
    return CARLA_3CLASS_SEGMENTATION_LABELS if args.seg_class_scheme == "carla_3class" else VOC_SEGMENTATION_LABELS


def prediction_mask_to_3class(mask: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    if args.seg_class_scheme == "carla_3class":
        ...
        return pred
    return map_voc_labels_to_3class(mask.astype(np.uint8, copy=False))
...
def _load_state_dict_if_requested(model: torch.nn.Module, path: str) -> None:
    if not path:
        return
    state = _checkpoint_state_dict(_load_checkpoint_blob(path))
...
def _build_raw_lraspp_model(args: argparse.Namespace) -> torch.nn.Module:
    ...
    weights = (
        ...
        if bool(args.seg_pretrained) and num_classes == len(VOC_SEGMENTATION_LABELS)
        else None
    )
    try:
    ...
    if num_classes != len(VOC_SEGMENTATION_LABELS):
        ...
        try:
            model.classifier = LRASPPHead(low_channels, high_channels, num_classes, inter_channels)
        except TypeError:
            model.classifier = LRASPPHead(low_channels, high_channels, num_classes)
...
def _quality_metric_columns(
    ...
    if pred_voc_mask is None or gt_3class is None:
        return columns
    pred_3class = prediction_mask_to_3class(pred_voc_mask.astype(np.uint8, copy=False), args)
...
def build_metrics_record(
    *,
    ...
    return {
        ...
        "ae_bottleneck_channels": int(args.ae_bottleneck_channels),
        "ae_spatial_stride": int(args.ae_spatial_stride),
        **_quality_metric_columns(mask, gt_3class, args),
...
def _mask_class_summary(mask: np.ndarray, max_items: int = 4) -> str:
    ...
    for label, count in foreground[:max_items]:
        name = (
            CARLA_3CLASS_SEGMENTATION_LABELS[label]
            if label < len(CARLA_3CLASS_SEGMENTATION_LABELS)
            else str(label)
        )
...
def run_demo(args: argparse.Namespace) -> None:
    random.seed(7)
    apply_trained_checkpoint_args(args)
