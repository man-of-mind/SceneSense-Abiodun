...
map_metrics_file: Optional[object] = None
map_metrics_writer: Optional[csv.DictWriter] = None
map_metrics_lock = threading.Lock()

MAP_INGEST_METRICS_FIELDS: Tuple[str, ...] = (
    "wall_time",
    "stream_id",
    "frame_id",
    "carla_timestamp",
    ...
    "t_map_publish_perf",
    "t_map_ingest_perf",
    "t_map_update_start_perf",
    "t_map_update_done_perf",
    "front_to_edge_ms",
    "tail_ms",
    ...
    "backbone_input_to_map_ingest_ms",
    "backbone_input_to_map_update_done_ms",
    "map_queue_ms",
    "map_service_ms",
    "capture_to_map_ingest_ms",
    "capture_to_map_update_done_ms",
    "map_update_delay_ms_config",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional CSV path for per-packet spatial-map ingest/update timing. "
            "Useful for uplink-only staleness measurements."
        ),
    )
    parser.add_argument(
        "--map-update-delay-ms",
        ...
        default=0.0,
        help=(
            "Artificial per-packet map-processing delay to emulate future association, "
            "occluder reasoning, or warning-policy compute."
        ),
    )
    parser.add_argument(
        "--display",
    ...
    parser.add_argument(
        "--focus-follow-stream-id",
        default="",
        help=(
            "Center the top-down ROI on the LIVE pose of this stream's anchor (e.g. a moving "
            "ego vehicle), following it as it drives. Takes precedence over --focus-traffic-light-ids. "
            "The ROI is a --focus-radius-m box around the ego's current position."
        ),
    )
    parser.add_argument(
        "--focus-follow-forward-bias",
        type=float,
        default=0.0,
        help=(
            "Shift the follow ROI this fraction of --focus-radius-m forward along the ego heading "
...
def _init_map_metrics_logger(path_value: str) -> None:
    global map_metrics_file, map_metrics_writer
    raw = str(path_value or "").strip()
    if not raw:
        return
    path = Path(raw)
    path.parent.mkdir(parents=True, exist_ok=True)
    map_metrics_file = path.open("w", newline="", encoding="utf-8")
    map_metrics_writer = csv.DictWriter(map_metrics_file, fieldnames=list(MAP_INGEST_METRICS_FIELDS))
    map_metrics_writer.writeheader()
    map_metrics_file.flush()
    print(f"[Metrics] Spatial-map ingest CSV: {path}")


def _append_map_metrics(row: Dict[str, object]) -> None:
    if map_metrics_writer is None or map_metrics_file is None:
        return
    clean = {key: row.get(key, "") for key in MAP_INGEST_METRICS_FIELDS}
...
def _close_map_metrics_logger() -> None:
    global map_metrics_file, map_metrics_writer
    with map_metrics_lock:
        if map_metrics_file is not None:
            try:
                map_metrics_file.flush()
            finally:
                map_metrics_file.close()
        map_metrics_file = None
        map_metrics_writer = None
...
def _follow_view_bounds(streams: Sequence[Dict[str, object]], now: float) -> Optional[Dict[str, object]]:
    """ROI that tracks a moving ego: a --focus-radius-m box around the live anchor pose of the
    stream named by --focus-follow-stream-id. Returns None if not configured or the stream is
    absent/stale so the caller can fall back to the static traffic-light focus."""
    cfg = _config()
    follow_id = str(getattr(cfg, "focus_follow_stream_id", "") or "").strip()
    if not follow_id:
        return None

    match = None
    for stream in streams:
        if str(stream.get("stream_id")) != follow_id:
            continue
        if now - float(stream.get("received_at", 0.0)) > float(cfg.stream_stale_s):
            continue
        match = stream
        break
    ...
    bias = float(getattr(cfg, "focus_follow_forward_bias", 0.0) or 0.0)
    if bias:  # push the box ahead of the car along its heading (CARLA: +x at yaw=0)
        yaw_rad = math.radians(yaw_deg)
        cx += bias * radius_m * math.cos(yaw_rad)
        cy += bias * radius_m * math.sin(yaw_rad)
    ...
    return {
        ...
        "padding_m": padding_m,
        "forward_bias": bias,
        "ego_pose": {"x": cx, "y": cy, "yaw_deg": yaw_deg},
        "bounds": {
            "x_min": float(cx - radius_m - padding_m),
            "x_max": float(cx + radius_m + padding_m),
            "y_min": float(cy - radius_m - padding_m),
            "y_max": float(cy + radius_m + padding_m),
        },
        "warning": None,
    }
...
def _build_spatial_map_snapshot() -> Dict[str, object]:
    ...
    # A live follow-ego ROI (if configured) takes precedence over the static traffic-light focus.
    focus_view = _follow_view_bounds(streams, now) or _focus_view_bounds(anchors)
...
def udp_listener_thread() -> None:
    ...
    while not STOP_EVENT.is_set():
        ...
        try:
            ...
            t_front_model_done_perf = _safe_float(timing.get("t_front_model_done_perf"), 0.0)
            t_front_payload_ready_perf = _safe_float(timing.get("t_front_payload_ready_perf"), 0.0)
            t_front_send_perf = _safe_float(timing.get("t_front_send_perf"), 0.0)
            t_edge_recv_perf = _safe_float(timing.get("t_edge_recv_perf"), 0.0)
            t_tail_done_perf = _safe_float(timing.get("t_tail_done_perf"), 0.0)
            t_map_publish_perf = _safe_float(timing.get("t_map_publish_perf"), 0.0)
            _append_map_metrics(
                {
                    "wall_time": datetime.now().isoformat(timespec="milliseconds"),
                    "stream_id": str(normalized.get("stream_id") or ""),
                    "frame_id": int(normalized.get("frame_id", 0)),
                    "carla_timestamp": _safe_float(normalized.get("carla_timestamp"), 0.0),
                    "object_count": int(normalized.get("object_count", 0)),
                    "packet_bytes": int(len(data)),
                    "t_capture_perf": t_capture_perf if t_capture_perf > 0.0 else "",
                    "t_front_start_perf": t_front_start_perf if t_front_start_perf > 0.0 else "",
                    "t_backbone_input_perf": (
                        t_backbone_input_perf if t_backbone_input_perf > 0.0 else ""
                    ),
                    "t_front_model_done_perf": (
                        t_front_model_done_perf if t_front_model_done_perf > 0.0 else ""
                    ),
                    "t_front_payload_ready_perf": (
                        t_front_payload_ready_perf if t_front_payload_ready_perf > 0.0 else ""
                    ),
                    "t_front_send_perf": t_front_send_perf if t_front_send_perf > 0.0 else "",
                    "t_edge_recv_perf": t_edge_recv_perf if t_edge_recv_perf > 0.0 else "",
                    "t_tail_done_perf": t_tail_done_perf if t_tail_done_perf > 0.0 else "",
                    "t_map_publish_perf": t_map_publish_perf if t_map_publish_perf > 0.0 else "",
                    "t_map_ingest_perf": float(t_map_ingest_perf),
                    "t_map_update_start_perf": float(t_map_update_start_perf),
                    "t_map_update_done_perf": float(t_map_update_done_perf),
                    "front_to_edge_ms": timing.get("front_to_edge_ms", ""),
                    "tail_ms": timing.get("tail_ms", ""),
                    "edge_to_map_publish_ms": (
                        float((t_map_publish_perf - t_edge_recv_perf) * 1000.0)
                        if t_map_publish_perf > 0.0 and t_edge_recv_perf > 0.0
                        else ""
                    ),
                    "capture_to_backbone_input_ms": timing.get(
                        "capture_to_backbone_input_ms",
                        "",
                    ),
                    "sync_world_tick_ms": timing.get("sync_world_tick_ms", ""),
                    "camera_frame_wait_ms": timing.get("camera_frame_wait_ms", ""),
                    "radar_wait_ms": timing.get("radar_wait_ms", ""),
                    "rgb_convert_ms": timing.get("rgb_convert_ms", ""),
                    "camera_inverse_matrix_ms": timing.get("camera_inverse_matrix_ms", ""),
                    "radar_tensor_build_ms": timing.get("radar_tensor_build_ms", ""),
                    "camera_matrix_ms": timing.get("camera_matrix_ms", ""),
                    "camera_transform_payload_ms": timing.get(
                        "camera_transform_payload_ms",
                        "",
                    ),
                    "pre_model_other_ms": timing.get("pre_model_other_ms", ""),
                    "capture_pipeline_queue_wait_ms": timing.get(
                        "capture_pipeline_queue_wait_ms",
                        "",
                    ),
                    "capture_pipeline_queue_depth": timing.get(
                        "capture_pipeline_queue_depth",
                        "",
                    ),
                    "model_preprocess_ms": timing.get("model_preprocess_ms", ""),
                    "front_backbone_ms": timing.get("front_backbone_ms", ""),
                    "feature_serialize_ms": timing.get("feature_serialize_ms", ""),
                    "backbone_input_to_front_send_ms": timing.get(
                        "backbone_input_to_front_send_ms",
                        "",
                    ),
                    "backbone_input_to_edge_recv_ms": timing.get(
                        "backbone_input_to_edge_recv_ms",
                        "",
                    ),
                    "backbone_input_to_tail_done_ms": timing.get(
                        "backbone_input_to_tail_done_ms",
                        "",
                    ),
                    "backbone_input_to_map_publish_ms": timing.get(
                        "backbone_input_to_map_publish_ms",
                        "",
                    ),
                    "backbone_input_to_map_ingest_ms": (
                        float((t_map_ingest_perf - t_backbone_input_perf) * 1000.0)
                        if t_backbone_input_perf > 0.0
                        else ""
                    ),
                    "backbone_input_to_map_update_done_ms": (
                        float((t_map_update_done_perf - t_backbone_input_perf) * 1000.0)
                        if t_backbone_input_perf > 0.0
                        else ""
                    ),
                    "map_queue_ms": (
                        float((t_map_ingest_perf - t_map_publish_perf) * 1000.0)
                        if t_map_publish_perf > 0.0
                        else ""
                    ),
                    "map_service_ms": float((t_map_update_done_perf - t_map_update_start_perf) * 1000.0),
                    "capture_to_map_ingest_ms": (
                        float((t_map_ingest_perf - t_capture_perf) * 1000.0)
                        if t_capture_perf > 0.0
                        else ""
                    ),
                    "capture_to_map_update_done_ms": (
                        float((t_map_update_done_perf - t_capture_perf) * 1000.0)
                        if t_capture_perf > 0.0
                        else ""
                    ),
                    "map_update_delay_ms_config": float(delay_ms),
                }
            )
        except Exception as exc:
...
def main() -> None:
    ...
    _init_map_metrics_logger(str(CONFIG.ingest_metrics_csv))
