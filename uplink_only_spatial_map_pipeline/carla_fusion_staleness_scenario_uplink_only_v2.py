...
# Standalone host and container invocations can inherit both roots in an
# arbitrary PYTHONPATH order.  Force the versioned UE-A2 runtime to resolve
# the current abiodun codec/scripts before similarly named legacy files in the
# parent neu_collab directory.
...
for _dependency_name, _dependency_module in (
    ("od_demo", od_demo),
    ("od_collect", od_collect),
    ("seg_demo", seg_demo),
    ("trained_seg_demo", trained_seg_demo),
    ("pole_client", pole_client),
):
    _dependency_file = Path(str(getattr(_dependency_module, "__file__", ""))).resolve()
    if not _dependency_file.is_relative_to(_ABIODUN_ROOT):
        raise ImportError(
            "UE-A2 runtime resolved a legacy dependency outside the current "
            f"abiodun tree: {_dependency_name}={_dependency_file}"
        )
...
EDGE_UPLINK_METRICS_FIELDS = (
    ...
    "stream_id",
    "ue_profile_id",
    "ue_action_contract_sha256",
    "ue_profile_identity_status",
    "carla_timestamp",
    "result_mode",
    "object_count",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help="UE-A2 registry CSV used to resolve one fail-closed fixed split profile.",
    )
    parser.add_argument(
        "--ue-profile-id",
        default="",
        help="Exact registered split profile ID selected for this fixed-profile process.",
    )
    parser.add_argument(
        "--require-ue-profile-binding",
        action="store_true",
        help="Reject startup unless registry, profile, checkpoint, codec, q, and decoder bind exactly.",
    )
...
class EdgeUplinkMetricsLogger:
    ...
    def write_summary(self, summary: Dict[str, object]) -> None:
        ...
        encoded = json.dumps(enriched, indent=2, sort_keys=True).encode("utf-8")
        temporary_path = self.summary_path.with_name(self.summary_path.name + ".tmp")
        with self._lock:
...
class CameraSideFusionInference:
    def __init__(
        ...
        *,
        ...
        device: torch.device,
        model_input_size: Tuple[int, int],
        registered_profile: Optional[RegisteredSplitProfile] = None,
    ) -> None:
        self.model = model
        self.sender = sender
        ...
        self.profile_identity = (
            dict(registered_profile.wire_identity)
    ...
    def process(
        ...
        *,
        ...
        with torch.inference_mode():
            ...
            if self.registered_profile is not None:
                validate_declared_feature_shapes(
                    self.registered_profile,
                    {
                ...
                for name, tensor in features.items():
                    if not bool(torch.isfinite(tensor).all()):
                        raise SplitWireContractError(
                            "NATIVE_FEATURE_NONFINITE",
                            f"profile={self.registered_profile.profile_id} level={name}",
                        )
        ...
        if self.profile_identity is not None:
            payload["profile_identity"] = dict(self.profile_identity)
        ...
        return {
            ...
            **prep_timing,
            ...
            "ue_profile_id": (
                self.registered_profile.profile_id
                if self.registered_profile is not None
                else ""
            ),
            "ue_action_contract_sha256": (
                self.registered_profile.action_contract_sha256
                if self.registered_profile is not None
                else ""
            ),
        }
...
class FusionRemoteInferenceWorker(threading.Thread):
    def __init__(
        ...
        *,
        ...
        self._processed = 0
        self._profile_identity_accepted = 0
        self._profile_identity_rejected = 0
        self._profile_rejection_reasons: Counter[str] = Counter()
        self._last_profile_rejection: Optional[Dict[str, object]] = None
        # Queue mode has a dedicated UDP receive thread while this worker
        # performs tail inference.  Both paths can update or snapshot the
        # profile counters, so protect the mutable Counter/dict as one state.
    ...
    def summary(self) -> Dict[str, object]:
        with self._profile_state_lock:
            ...
            receive_dropped = int(self._receive_dropped)
            udp_partial_messages_dropped = int(self._udp_partial_messages_dropped)
            profile_identity_accepted = int(self._profile_identity_accepted)
            profile_identity_rejected = int(self._profile_identity_rejected)
            profile_rejection_reasons = dict(
                sorted(self._profile_rejection_reasons.items())
            )
            last_profile_rejection = (
                dict(self._last_profile_rejection)
                if self._last_profile_rejection is not None
                else None
            )
        return {
            "label": self.label,
            ...
            "edge_receive_queue_dropped": receive_dropped,
            "udp_pending_messages": len(getattr(self.receiver, "_pending", {}) or {}),
            "udp_partial_messages_dropped": udp_partial_messages_dropped,
            "result_mode": self.result_mode,
            "registered_profile_id": (
                self.registered_profile.profile_id if self.registered_profile is not None else ""
            ),
            "registered_profile_identity": (
                dict(self.registered_profile.wire_identity)
                if self.registered_profile is not None
                else None
            ),
            "registered_registry_sha256": (
                self.registered_profile.registry_sha256
                if self.registered_profile is not None
                else ""
            ),
            "profile_identity_accepted": profile_identity_accepted,
            "profile_identity_rejected": profile_identity_rejected,
            "profile_rejection_reasons": profile_rejection_reasons,
            "last_profile_rejection": last_profile_rejection,
        }

    def _write_summary_snapshot(self) -> None:
        if self.edge_metrics_logger is not None:
            self.edge_metrics_logger.write_summary(self.summary())

    def _accept_profile_payload(self, payload: object) -> bool:
        if self.registered_profile is None:
            return True
        try:
            validate_feature_payload(self.registered_profile, payload)
        except SplitWireContractError as exc:
            frame_id = payload.get("frame_id", -1) if isinstance(payload, dict) else -1
            rejection = {
                "wall_time_iso": datetime.now().isoformat(timespec="milliseconds"),
                "frame_id": _safe_int(frame_id, -1),
                "code": exc.code,
                "detail": exc.detail,
            }
            with self._profile_state_lock:
                self._profile_identity_rejected += 1
                self._profile_rejection_reasons[exc.code] += 1
                self._last_profile_rejection = rejection
            self._write_summary_snapshot()
            print(
                "[UE-A2] PROFILE_CONTRACT_MISMATCH "
                f"frame={frame_id} code={exc.code} detail={exc.detail}",
                file=sys.stderr,
            )
            return False
        with self._profile_state_lock:
            self._profile_identity_accepted += 1
        return True
            ...
            if payload is None:
                if self.log_every > 0:
                    ...
                    if now - self._last_wait_log >= 5.0:
                        ...
                        self._last_wait_log = now
                continue
            if not self._accept_profile_payload(payload):
                continue
    ...
    def _process_payload(
        ...
        try:
            ...
            if self.registered_profile is not None:
                result["profile_identity"] = dict(self.registered_profile.wire_identity)
            ...
            if self.result_mode == "none":
                ...
                if self.registered_profile is not None:
                    send_payload["profile_identity"] = dict(
                        self.registered_profile.wire_identity
                    )
            else:
                send_payload = result
            ...
            with self._profile_state_lock:
                self._processed += 1
                processed_count = int(self._processed)
            if processed_count == 1 or (
                self.log_every > 0 and processed_count % self.log_every == 0
            ):
                self._write_summary_snapshot()
            ...
            if self.edge_metrics_logger is not None:
                observed_identity = (
                    payload.get("profile_identity")
                    if isinstance(payload.get("profile_identity"), dict)
                    else {}
                )
                self.edge_metrics_logger.append(
                    {
                        ...
                        "ue_profile_id": str(observed_identity.get("profile_id") or ""),
                        "ue_action_contract_sha256": str(
                            observed_identity.get("action_contract_sha256") or ""
                        ),
                        "ue_profile_identity_status": (
                            "ACCEPTED_REGISTERED"
                            if self.registered_profile is not None
    ...
    def _run_back_half(self, payload: Dict[str, object]) -> Dict[str, object]:
        ...
        if self.registered_profile is not None:
            ...
            for name, tensor in features.items():
                if not bool(torch.isfinite(tensor).all()):
                    raise SplitWireContractError(
                        "EDGE_DECODED_FEATURE_NONFINITE",
                        f"profile={self.registered_profile.profile_id} level={name}",
                    )
        ...
        return {
            ...
            "objects": objects,
            **(
                {"profile_identity": dict(self.registered_profile.wire_identity)}
                if self.registered_profile is not None
                else {}
            ),
        }
...
def _validated_result_profile_identity(
    payload: object,
    registered_profile: Optional[RegisteredSplitProfile],
) -> Optional[Dict[str, object]]:
    if registered_profile is None:
        return None
    observed_identity = payload.get("profile_identity") if isinstance(payload, dict) else None
    return validate_wire_identity(observed_identity, registered_profile.wire_identity)
...
class CameraResultReceiver(threading.Thread):
    def __init__(
        ...
        *,
        ...
        self.registered_profile = registered_profile
        self.profile_identity_accepted = 0
        self.profile_identity_rejected = 0

    def _accept_result_payload(self, payload: object) -> bool:
        if self.registered_profile is None:
            return True
        try:
            _validated_result_profile_identity(payload, self.registered_profile)
        except SplitWireContractError as exc:
            self.profile_identity_rejected += 1
            frame_id = payload.get("frame_id", -1) if isinstance(payload, dict) else -1
            print(
                "[UE-A2] RESULT_PROFILE_CONTRACT_MISMATCH "
                f"frame={frame_id} code={exc.code} detail={exc.detail}",
                file=sys.stderr,
            )
            return False
        self.profile_identity_accepted += 1
        return True
    ...
    def run(self) -> None:
        while not self.stop_event.is_set():
            ...
            if payload is None:
                continue
            if not self._accept_result_payload(payload):
                continue
...
def _resolve_registered_ue_profile(
    args: argparse.Namespace,
) -> Optional[RegisteredSplitProfile]:
    """Resolve and validate one fixed UE-A2 profile before model construction."""

    existing = getattr(args, "_ue_registered_profile", None)
    if isinstance(existing, RegisteredSplitProfile):
        return existing
    registry_raw = str(getattr(args, "ue_profile_registry_csv", "") or "").strip()
    profile_id = str(getattr(args, "ue_profile_id", "") or "").strip()
    required = bool(getattr(args, "require_ue_profile_binding", False))
    if not required and not registry_raw and not profile_id:
        return None
    if not required:
        raise SplitWireContractError(
            "STRICT_PROFILE_BINDING_REQUIRED",
            "--ue-profile-registry-csv/--ue-profile-id require --require-ue-profile-binding",
        )
    if not registry_raw or not profile_id:
        raise SplitWireContractError(
            "REGISTERED_PROFILE_ARGUMENTS_INCOMPLETE",
            "strict mode requires both --ue-profile-registry-csv and --ue-profile-id",
        )
    ...
    setattr(args, "_ue_registered_profile", profile)
    setattr(args, "_ue_profile_binding_audit", audit)
    print(
        "[UE-A2] strict profile binding accepted "
        f"profile={profile.profile_id} "
        f"action_contract={profile.action_contract_sha256} "
        f"checkpoint={audit['checkpoint_sha256']}"
    )
    return profile
...
def load_fusion_model(
    ...
    registered_profile = _resolve_registered_ue_profile(args)
    checkpoint_path = _resolve_fusion_checkpoint_path(args)
    ...
    if registered_profile is not None and ae_ckpt_path:
        raise SplitWireContractError(
            "EXTERNAL_AE_OVERRIDE_FORBIDDEN",
            f"profile={registered_profile.profile_id} ae_checkpoint={ae_ckpt_path}",
        )
    if ae_ckpt_path:
        import sys as _sys
...
def _normalize_spatial_objects(
    ...
    *,
    ...
    for index, obj in enumerate(objects):
        raw_class_name = str(obj.get("class_name", "") or "").strip().lower()
        if raw_class_name in {"person", "pedestrian"}:
            map_object_type = "Pedestrian"
        elif raw_class_name in {"vehicle", "car", "truck", "bus"}:
            map_object_type = "Vehicle"
        else:
            map_object_type = "Unknown"
    ...
    def write_manifest(
        ...
        *,
        ...
        manifest = {
            ...
            "ue_profile_binding": _json_safe(
                getattr(self.args, "_ue_profile_binding_audit", None)
            ),
            "output_files": {
        ...
        if isinstance(resolved_config, dict):
            # The dataclass contains a MappingProxy row and is intentionally not
            # serialized via ``str``.  Its exact, typed audit record is stored
            # under the stable public key below.
            resolved_config.pop("_ue_registered_profile", None)
            resolved_config.pop("_ue_profile_binding_audit", None)
...
class SpatialMapResultPublisher:
    ...
    def publish(
        ...
        *,
        ...
        remote_identity = (
            (remote_stats or {}).get("profile_identity")
            if isinstance((remote_stats or {}).get("profile_identity"), dict)
            else None
        )
        if remote_identity is not None:
            payload["profile_identity"] = dict(remote_identity)
    ...
    def publish_from_payload(
        ...
        *,
        ...
        source_identity = source_payload.get("profile_identity")
        if isinstance(source_identity, dict):
            payload["profile_identity"] = dict(source_identity)
...
def run_back_only(args: argparse.Namespace) -> None:
    ...
    registered_profile = getattr(args, "_ue_registered_profile", None)
    ...
    remote_worker = FusionRemoteInferenceWorker(
        ...
        registered_profile=(
            registered_profile
            if isinstance(registered_profile, RegisteredSplitProfile)
            else None
        ),
    )
        ...
        while True:
            ...
            if result is not None:
                remote_stats = {
                    ...
                    "profile_identity": (
                        _validated_result_profile_identity(result, registered_profile)
                    ),
                }
                mask = result.get("mask") if isinstance(result.get("mask"), np.ndarray) else None
